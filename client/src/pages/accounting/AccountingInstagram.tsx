...
import { ChevronLeft, ChevronRight, TrendingUp, TrendingDown, Wallet, Check, Loader2 } from "lucide-react";
...
export default function AccountingInstagram() {
  ...
  const ledgerQ  = trpc.accounting.accInstagramLedger.useQuery({ page, pageSize: PAGE_SIZE }, {
    refetchOnWindowFocus: false,
    placeholderData: (prev) => prev,
  });
  const addMut = trpc.accounting.addAccInstagram.useMutation({
    onSuccess: () => {
  ...
  const instagram = reportsQ.data?.instagram;
  const { rows = [], total = 0 } = ledgerQ.data ?? {};
  const totalPages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const net = instagram?.net ?? 0;
      ...
      <div className="space-y-5" dir="rtl">
        ...
        <section className="rounded-[24px] border border-slate-200 bg-white p-4 lg:p-5">
          <div className="flex gap-4">
            ...
            {/* الإجماليات + إضافة قيد stacked — fills available space */}
            <div className="flex flex-1 flex-col gap-3 min-w-0">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-2">
                  {([
                    { label: "معاه (وارد)",  val: instagram?.totalIn,  cls: "text-emerald-700", icon: TrendingUp   },
                    { label: "منه (صادر)",   val: instagram?.totalOut, cls: "text-rose-700",    icon: TrendingDown },
                    { label: "الرصيد",        val: net,                 cls: net >= 0 ? "text-pink-700" : "text-rose-700", icon: Wallet },
                  ] as const).map((m) => {
                    const Icon = m.icon;
                    return (
              ...
              <div className="flex flex-col gap-2" dir="rtl">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button"
                    disabled={addMut.isPending || (!entryIn && !entryOut)}
        ...
        <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <div>
              <h2 className="text-sm font-bold text-slate-900">حركات انستاباي</h2>
              <p className="mt-1 text-xs text-slate-500">كل الحركات الواردة والصادرة.</p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {ledgerQ.isLoading && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-500">جاري التحميل...</td></tr>
                )}
                {!ledgerQ.isLoading && rows.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-sm text-slate-500">لا توجد حركات</td></tr>
                )}
                {rows.map((row) => (
