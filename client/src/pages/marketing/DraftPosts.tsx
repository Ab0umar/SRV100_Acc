...
import { FileText, ImageIcon, Loader2, RefreshCw, Send, Trash2 } from "lucide-react";
...
export default function DraftPosts() {
  ...
  const publishMutation = trpc.marketing.publishPost.useMutation({
    ...
    onError: (err: { message: string }) => { toast.error(err.message); setPublishId(null); },
  });
            ...
            <div key={post.id} className="rounded-xl border border-border bg-card overflow-hidden">
              ...
              {post.imageUrl ? (
                ...
                <div className="flex h-28 items-center justify-center bg-muted/30 border-b border-border">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">لا توجد صورة — اضغط "توليد صورة"</p>
                  </div>
                </div>
              ) : null}
              ...
              <div className="p-4 space-y-2">
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge className="shrink-0 bg-muted text-muted-foreground text-xs">مسودة</Badge>
                </div>

                {/* Content */}
                {post.content && (
                  <p className="line-clamp-3 text-sm text-foreground/80 leading-relaxed">{post.content}</p>
                )}
                ...
                {/* CTA */}
                {post.cta && <p className="text-xs font-medium text-foreground/80">{post.cta}</p>}
                ...
                {post.imagePrompt && !post.imageUrl && (
                  <div className="rounded-md border border-dashed border-border bg-muted/20 px-3 py-2">
                    <p className="text-xs text-muted-foreground line-clamp-2">
                      <span className="font-medium">وصف الصورة: </span>{post.imagePrompt}
                    </p>
                  </div>
                )}
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  {post.imagePrompt && (
                    <Button
                      ...
                      onClick={() => { setImageGenId(post.id); generateImageMutation.mutate({ postId: post.id }); }}
                      disabled={generateImageMutation.isPending && imageGenId === post.id}
                    >
                      {generateImageMutation.isPending && imageGenId === post.id
                  ...
                  <Button
                    ...
                    variant="default"
                    className="gap-1.5 text-xs"
                    onClick={() => { setPublishId(post.id); publishMutation.mutate({ id: post.id }); }}
