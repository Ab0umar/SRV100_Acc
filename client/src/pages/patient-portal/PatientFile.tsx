import { RefreshCw, Stethoscope, ShieldAlert, BadgeInfo, Glasses, Pill, ScanLine, CalendarDays, UserRound, Phone, MapPin, Calendar } from "lucide-react";
...
function DetailItem({ label, value, icon: Icon }: { label: string; value?: string | number | null; icon?: any }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-[#f0f5fa] py-3.5 last:border-0 last:pb-0">
      ...
      <span className="text-sm font-bold text-foreground text-left">{String(value)}</span>
    </div>
  );
}
        ...
        {data && (
          <>
            ...
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-primary">أهلاً بك، {data.fullName.split(" ")[0]} 👋</h2>
              <p className="text-xs text-muted-foreground">مرحباً بك في بوابتك الطبية الإلكترونية لمتابعة ملفك وحجوزاتك.</p>
            </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-lg font-bold text-foreground">{data.fullName}</h3>
                      {data.status && (
                    ...
                    <p className="text-xs text-muted-foreground leading-none">
                      كود المريض: <span className="font-bold text-[#003D82] font-mono">{data.patientCode}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 w-full md:w-auto md:min-w-[360px]">
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
                    <p className="text-[10px] font-semibold text-muted-foreground">الجنس</p>
                    <p className="text-xs font-bold text-foreground mt-0.5">
                      {data.gender ? GENDER_LABEL[data.gender] ?? data.gender : "غير محدد"}
                    </p>
                  </div>
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
                    <p className="text-[10px] font-semibold text-muted-foreground">العمر</p>
                    <p className="text-xs font-bold text-foreground mt-0.5">
                      {data.age ? `${data.age} سنة` : "غير محدد"}
                    </p>
                  </div>
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
                    <p className="text-[10px] font-semibold text-muted-foreground">آخر زيارة</p>
                    <p className="text-xs font-bold text-foreground mt-0.5">
            ...
            <div className="space-y-2.5">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">الوصول السريع للخدمات</h3>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
                ...
                <Link href="/my/refraction">
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-4 text-center space-y-3 shadow-xs hover:border-primary/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-center items-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-foreground">مقاس النظارة</p>
                      <p className="text-[10px] text-muted-foreground">آخر قياسات البصر</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/my/prescription">
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-4 text-center space-y-3 shadow-xs hover:border-primary/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-center items-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-foreground">روشتاتي الدوائية</p>
                      <p className="text-[10px] text-muted-foreground">العلاجات والجرعات</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/my/scans">
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-4 text-center space-y-3 shadow-xs hover:border-primary/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-center items-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-foreground">الأشعة والفحوصات</p>
                      <p className="text-[10px] text-muted-foreground">تقارير وصور العين</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/my/book">
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-4 text-center space-y-3 shadow-xs hover:border-primary/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-center items-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-foreground">حجز موعد جديد</p>
                      <p className="text-[10px] text-muted-foreground">اختيار موعد بالعيادة</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/my/bookings" className="col-span-2 md:col-span-1">
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-4 text-center space-y-3 shadow-xs hover:border-primary/30 transition-all duration-200 cursor-pointer h-full flex flex-col justify-center items-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-bold text-foreground">مواعيدي السابقة</p>
                      <p className="text-[10px] text-muted-foreground">قائمة الحجوزات وحالتها</p>
                    </div>
                  </div>
                </Link>
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="bg-white border border-[#dbe7f4] rounded-2xl p-5 shadow-xs space-y-4">
                <div className="border-b border-[#f0f5fa] pb-2">
                  <h4 className="text-sm font-bold text-foreground">البيانات الشخصية والاتصال</h4>
                  <p className="text-[10px] text-muted-foreground">المسجلة في استقبال المركز</p>
                </div>
                <div className="space-y-0.5">
                  <DetailItem label="تاريخ الميلاد" value={formatArabicDate(data.dateOfBirth)} icon={Calendar} />
                  <DetailItem label="رقم الموبايل" value={data.phone} icon={Phone} />
                  <DetailItem label="العنوان" value={data.address} icon={MapPin} />
                  <DetailItem label="فئة الخدمة" value={data.serviceType ? SERVICE_LABEL[data.serviceType] ?? data.serviceType : null} icon={BadgeInfo} />
                </div>
              </div>
              ...
              <div className="bg-white border border-[#dbe7f4] rounded-2xl p-5 shadow-xs space-y-4">
                <div className="border-b border-[#f0f5fa] pb-2">
                  <h4 className="text-sm font-bold text-foreground">التاريخ المرضي والحساسية</h4>
                  <p className="text-[10px] text-muted-foreground">معلومات طبية هامة لسلامتك</p>
                </div>
