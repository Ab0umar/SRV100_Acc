...
export default function LeaveManagement() {
  const [filter, setFilter] = useState({ empCd: "", from: firstOfYear, to: today });
  ...
  const approveMut = trpc.attendance.approveLeave.useMutation({
    onSuccess: () => { leavesQuery.refetch(); toast.success("تم الاعتماد"); },
    onError: (e) => toast.error("خطأ: " + e.message),
  });

  const deleteMut = trpc.attendance.deleteLeave.useMutation({
    onSuccess: () => { leavesQuery.refetch(); toast.success("تم الحذف"); },
    onError: (e) => toast.error("خطأ: " + e.message),
  });
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto" dir="rtl">
      ...
      <Card className="mb-4">
        <CardContent className="pt-4">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <Button onClick={() => leavesQuery.refetch()} variant="outline">بحث</Button>
      ...
      {showForm && (
        <Card className="mb-4">
          <CardHeader><CardTitle>إضافة إجازة جديدة</CardTitle></CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as LeaveType })} className="w-full px-3 py-2 border rounded-md">
                  {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">من تاريخ</label>
                <input type="date" value={form.dateFrom} onChange={(e) => setForm({ ...form, dateFrom: e.target.value })} className="w-full px-3 py-2 border rounded-md" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">إلى تاريخ</label>
                <input type="date" value={form.dateTo} onChange={(e) => setForm({ ...form, dateTo: e.target.value })} className="w-full px-3 py-2 border rounded-md" />
              </div>
      ...
      <Card>
        ...
        <CardContent>
          {leavesQuery.isLoading ? (
            ...
            <div className="text-center py-8 text-gray-500">لا توجد إجازات</div>
          ) : (
                  ...
                  {leaves.map((l: any) => (
                    <tr key={l.id} className="border-b hover:bg-gray-50">
                      <td className="py-2 px-4">
                        <span className="font-mono text-xs text-gray-500 block">{l.empCd}</span>
                        <span>{l.empName ?? "—"}</span>
                      </td>
                      <td className="py-2 px-4">{l.dateFrom}</td>
                      <td className="py-2 px-4">{l.dateTo}</td>
                      <td className="py-2 px-4 font-medium">{dayCount(l.dateFrom, l.dateTo)}</td>
                      <td className="py-2 px-4">{TYPE_LABELS[l.type as LeaveType] ?? l.type}</td>
                      <td className="py-2 px-4">
                        {l.approved
                          ? <span className="text-green-600 font-medium flex items-center gap-1"><CheckCircle size={14} />معتمدة</span>
                          : <span className="text-yellow-600 font-medium">انتظار</span>}
                      </td>
                      <td className="py-2 px-4 text-gray-500 text-xs">{l.note ?? "—"}</td>
                      <td className="py-2 px-4">
                        <div className="flex gap-1">
                          {!l.approved && (
                            <Button variant="outline" size="sm" onClick={() => approveMut.mutate({ leaveId: l.id })} disabled={approveMut.isPending} className="text-green-600 border-green-300 h-7 px-2">
                              اعتماد
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => deleteMut.mutate({ leaveId: l.id })} disabled={deleteMut.isPending}>
