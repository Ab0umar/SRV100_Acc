...
export default function UserMappings() {
  ...
  if (query.isLoading) return <div className="p-4 space-y-2"><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></div>;
  ...
  return (
    <div className="p-4" dir="rtl">
      <p className="mb-4 text-sm text-muted-foreground">
        اربط كل مستخدم بالرمز الذي يظهر في جهاز البصمة (empCd) حتى تظهر له بيانات حضوره في صفحة "حضوري".
      </p>
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-border bg-muted/40">
            <tr>
              ...
              <th className="px-3 py-2 text-right font-medium">رمز البصمة (empCd)</th>
              ...
              <tr key={u.id} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-3 py-2">
                  {editingId === u.id ? (
                    ...
                    <span className={u.empCd ? "font-mono text-foreground" : "text-muted-foreground"}>
                      {u.empCd ?? "—"}
                    </span>
                  )}
                </td>
                  ...
                  {editingId === u.id ? (
                    <div className="flex gap-1">
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-success"
                        ...
                        onClick={() => setMapping.mutate({ userId: u.id, empCd: editVal.trim() || null })}>
