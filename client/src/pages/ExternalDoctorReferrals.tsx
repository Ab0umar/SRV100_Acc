...
function CreateReferralForm({ onDone }: { onDone: () => void }) {
  ...
  const create = trpc.externalDoctors.createReferral.useMutation({
    onSuccess: () => { toast.success("Referral created"); onDone(); },
    ...
    <div className="space-y-3 rounded-xl border border-primary/20 bg-primary/5 p-4">
      ...
      <div className="flex gap-2">
        <Button
          ...
          onClick={() => create.mutate({ externalDoctorId: Number(doctorId), patientCode })}
        >
...
export default function ExternalDoctorReferrals() {
  ...
  const { data, isLoading, refetch } = trpc.externalDoctors.listReferrals.useQuery();
  ...
  const del = trpc.externalDoctors.deleteReferral.useMutation({
    onSuccess: () => { toast.success("Referral deleted"); void refetch(); },
    onError: (e) => toast.error(e.message),
  });
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Control which patients each external doctor can view</p>
          </div>
          <div className="flex gap-2">
        ...
        {creating && <CreateReferralForm onDone={() => { setCreating(false); void refetch(); }} />}

        {isLoading && (
          <div className="space-y-3">
            {[1, 2, 3].map((n) => <div key={n} className="h-14 animate-pulse rounded-xl bg-muted" />)}
          </div>
        )}
              ...
              <div key={r.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-card px-4 py-3">
                <div className="min-w-0 space-y-0.5">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold text-foreground">{r.patientName ?? r.patientCode}</p>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (confirm("Delete this referral?")) del.mutate({ id: r.id });
                    }}
                  >
