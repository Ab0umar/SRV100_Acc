              ...
              <div key={item.id ?? index} className="rounded-xl border border-[#e2edf7] bg-[#F4F8FB]/30 p-4 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-bold text-foreground">{v(item.medicationName)}</p>
                  </div>
...
export default function DoctorPatientImages() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "refractions" | "prescriptions" | "images">("profile");
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-xl font-bold text-primary">
              {data?.patientName ? `الملف الطبي للمريض: ${data.patientName}` : "ملف المريض"}
            </h2>
            <p className="text-xs text-muted-foreground font-mono">{patientCode}</p>
          </div>
          <Button variant="ghost" size="sm" onClick={() => void refetch()} className="text-muted-foreground cursor-pointer">
        ...
        {error && (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-8 text-center">
            ...
            <Button size="sm" variant="outline" onClick={() => void refetch()} className="cursor-pointer">إعادة المحاولة</Button>
          </div>
        )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-lg font-bold text-foreground">{data.patient.fullName}</h3>
                      {data.patient.status && (
                        <Badge className="bg-secondary/15 text-secondary border-0 hover:bg-secondary/20 font-bold px-2.5 py-0.5 rounded-lg text-[10px] pointer-events-none">
                          {STATUS_LABEL[data.patient.status] ?? data.patient.status}
                        </Badge>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground leading-none">
                      كود المريض: <span className="font-bold text-[#003D82] font-mono">{data.patient.patientCode}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 w-full md:w-auto md:min-w-[360px]">
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
                    ...
                    <p className="text-xs font-bold text-foreground mt-0.5">
                      {data.patient.gender ? GENDER_LABEL[data.patient.gender] ?? data.patient.gender : "غير محدد"}
                    </p>
                  </div>
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
                    <p className="text-[10px] font-semibold text-muted-foreground">العمر</p>
                    <p className="text-xs font-bold text-foreground mt-0.5">
                      {data.patient.age ? `${data.patient.age} سنة` : "غير محدد"}
                    </p>
                  </div>
                  <div className="bg-[#F4F8FB] border border-[#e2edf7] rounded-xl p-2.5 text-center">
              ...
              {activeTab === "profile" && (
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-5 shadow-xs space-y-4">
                    ...
                    <div className="space-y-0.5">
                      <DetailItem label="تاريخ الميلاد" value={formatArabicDate(data.patient.dateOfBirth)} icon={Calendar} />
                      <DetailItem label="رقم الموبايل" value={data.patient.phone} icon={Phone} />
                      <DetailItem label="العنوان" value={data.patient.address} icon={MapPin} />
                      <DetailItem label="فئة الخدمة" value={data.patient.serviceType ? SERVICE_LABEL[data.patient.serviceType] ?? data.patient.serviceType : null} icon={BadgeInfo} />
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-[#dbe7f4] rounded-2xl p-5 shadow-xs space-y-4">
                    <div className="border-b border-[#f0f5fa] pb-2">
                      ...
                      <p className="text-[10px] text-muted-foreground">معلومات طبية هامة لسلامة المريض</p>
                    </div>
