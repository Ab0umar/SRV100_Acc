...
describe("color token regressions", () => {
  ...
  it("keeps the login submit label readable on the navy action background", () => {
    ...
    expect(source).not.toContain("bg-primary text-sm font-bold text-card-foreground");
    expect(source).toContain("bg-primary text-sm font-bold text-primary-foreground");
  });
