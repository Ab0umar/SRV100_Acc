...
export default function AdminMigrations() {
  ...
  const pendingCount = useMemo(() => {
    return (migrationsQuery.data?.migrations ?? []).filter((m) => m.pending)
      .length;
  ...
  const handleRefresh = async () => {
    ...
    if (result.error) {
      toast.error(
        getTrpcErrorMessage(result.error, "تعذر تحديث قائمة الترحيلات"),
      );
      return;
    }
        ...
        <Card className="border-destructive/30 bg-destructive/10">
          ...
          <CardContent>
            <p className="text-destructive">
              أنت لا تملك صلاحية الوصول لهذه الصفحة.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  const appliedPct =
    migrationTotal > 0
      ? Math.min(100, Math.round((migrationApplied / migrationTotal) * 100))
      : null;
  ...
  return (
    ...
    >
            ...
            >
              <RefreshCcw
                className={cn(
                  ...
                  migrationsQuery.isFetching && "animate-spin",
                )}
              />
              تحديث القائمة
            </Button>
      ...
      {!migrationsQuery.isLoading &&
        migrationsQuery.data?.source === "journal" && (
          <Card className="border-warning/50 bg-warning/10 shadow-sm border-dashed">
            <CardContent className="flex items-center gap-3 p-3 text-warning text-xs font-bold">
              ...
              تنبيه: تم عرض الترحيلات من سجل Drizzle المحلي لتعذر الاتصال
              المباشر.
              {migrationsQuery.data.dbError && (
          ...
          <Table dir="rtl" className="text-right">
            <TableHeader className="sticky top-0 z-10 bg-primary/5 backdrop-blur-sm shadow-sm">
              <TableRow className="hover:bg-transparent border-b-primary/10 h-11">
                <TableHead className="min-w-[200px] px-6 font-bold text-primary text-xs">
                  ملف الترحيل (SQL)
                </TableHead>
                <TableHead className="px-4 font-bold text-primary text-xs">
                  تاريخ التطبيق
                </TableHead>
                <TableHead className="text-center font-bold text-primary text-xs">
                  السجلات
                </TableHead>
                <TableHead className="text-center font-bold text-primary text-xs">
                  الحالة
            ...
            <TableBody>
              ...
              {rows.map((migration: MigrationRow, idx: number) => (
                ...
                >
                  <TableCell className="px-6 py-3 font-mono text-[11px] font-bold text-foreground/80 break-all">
                    {migration.name}
                  </TableCell>
                  <TableCell className="whitespace-nowrap py-3 text-[11px] font-medium text-muted-foreground tabular-nums">
                    {formatAt(migration.appliedAt)}
                  </TableCell>
                  <TableCell className="text-center text-muted-foreground py-3 opacity-30">
                    —
                  </TableCell>
                  <TableCell className="text-center py-3">
                    <Badge
                      variant={migration.pending ? "destructive" : "secondary"}
                      className={cn(
          ...
          <CardContent className="flex flex-col items-center justify-center gap-4 py-8 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="max-w-md text-[11px] text-muted-foreground leading-relaxed">
                ...
                لاستيراد بيانات Excel/CSV، يرجى استخدام أدوات المزامنة المتخصصة
                في صفحة الإدارة.
              </p>
            </div>
          </CardContent>
