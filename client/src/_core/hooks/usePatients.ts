...
export const usePatients = () => {
  ...
  const getPatients = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(getApiUrl("/api/medical/patients"), {
        method: "GET",
        headers: {
  ...
  const createPatient = useCallback(
    async (patientData: Patient) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "خطأ في إضافة المريض";
        setError(message);
        toast.error(message);
        return null;
      } finally {
  ...
  const updatePatient = useCallback(
    async (patientId: number, patientData: Partial<Patient>) => {
      setLoading(true);
      setError(null);
      try {
        const response = await fetch(
          getApiUrl(`/api/medical/patients/${patientId}`),
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
        ...
        const message =
          err instanceof Error ? err.message : "خطأ في تحديث المريض";
        setError(message);
        toast.error(message);
        return null;
      } finally {
  ...
  const deletePatient = useCallback(
    async (patientId: number) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "خطأ في حذف المريض";
        setError(message);
        toast.error(message);
        return false;
      } finally {
  ...
  const searchPatients = useCallback(async (searchTerm: string) => {
    ...
    try {
      const response = await fetch(
        getApiUrl(
          `/api/medical/patients/search?q=${encodeURIComponent(searchTerm)}`,
        ),
        {
          method: "GET",
          headers: {
