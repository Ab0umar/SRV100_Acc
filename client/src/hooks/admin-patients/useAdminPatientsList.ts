...
export function useAdminPatientsList() {
  ...
  >({});
  const [manualLockOverrides, setManualLockOverrides] = useState<
  ...
  const serviceDirectoryQuery = trpc.medical.getServicesFromDb.useQuery(
    undefined,
    {
      refetchOnWindowFocus: false,
      staleTime: 60 * 60 * 1000,
  ...
  const savePatientPageStateMutation =
    trpc.medical.savePatientPageState.useMutation();
  const deletePatientFromMssqlMutation =
    trpc.medical.deletePatientFromMssql.useMutation();
  ...
  const statsFilterInput = useMemo(
    ...
    [doctorFilter, serviceTypeFilter, locationFilter, dateFrom, dateTo],
  );
  ...
  const patientsPayload = (patientsQuery.data ?? {
    rows: [],
    hasMore: false,
    nextCursor: null,
  }) as {
  ...
  const hasMore =
    doctorFilter === "all" ? Boolean(patientsPayload.hasMore) : false;
  const nextCursor =
    doctorFilter === "all" ? (patientsPayload.nextCursor ?? null) : null;
  ...
  const years = useMemo(() => {
    ...
    for (const patient of patients) {
      const yearMonth = getYearMonth(
        (patient as PatientRow & { lastVisit?: unknown }).lastVisit,
      );
  ...
  const monthStats = (patientStatsBundleQuery.data?.currentMonth ?? {
    total: 0,
    center: 0,
    external: 0,
    lasik: 0,
  }) as PatientStats;
  const yearStats = (patientStatsBundleQuery.data?.yearly ?? {
    total: 0,
    center: 0,
    external: 0,
    lasik: 0,
  ...
  const serviceCodeToType = useMemo(() => {
    ...
    for (const item of list) {
      ...
      const type = normalizeSheetTypeChoice(
        (item as { defaultSheet?: string; serviceType?: string })
          .defaultSheet ??
  ...
  const getRowServiceCode = (patient: PatientRow) =>
    normalizeServiceCodeForSearch(
      String(
        (patient as PatientRow & { __serviceCodeSingle?: string })
          .__serviceCodeSingle ??
          patient.serviceCode ??
          "",
      ).trim(),
  ...
  const getRowSheetType = (patient: PatientRow): SheetTypeChoice => {
    ...
    const mappedOverride = rowServiceCode
      ? normalizeSheetTypeChoice(
          patient.serviceSheetTypeByCode?.[rowServiceCode],
        )
      : "";
    if (mappedOverride) return mappedOverride;
    const mappedDefault = rowServiceCode
      ? normalizeSheetTypeChoice(serviceCodeToType.get(rowServiceCode))
      : "";
  ...
  const filteredPatients = useMemo(() => {
    ...
    const sorted = [...patients].sort((a, b) => {
      ...
      return String(a.patientCode ?? "").localeCompare(
        String(b.patientCode ?? ""),
        "ar",
      );
    });
    ...
    const result = sorted.flatMap((patient) => {
      ...
      const serviceName = String(
        serviceCodeToLabel.get(primaryCode) ?? "",
      ).trim();
      const serviceType = String(serviceCodeToType.get(primaryCode) ?? "")
        .trim()
        .toLowerCase();
      ...
      if (
        cached &&
        cached.patient === patient &&
        cached.__serviceCodeSingle === primaryCode
      ) {
        return [cached];
      }
  ...
  const isExpanded = useCallback(
    (patientId: number) => expandedPatients.has(patientId),
    [expandedPatients],
  );
  ...
  const getDraft = useCallback(
    (patient: PatientRow): PatientDraft => {
  ...
  const setDraftField = useCallback(
    (patient: PatientRow, field: keyof PatientDraft, value: string) => {
      const rowKey = getPatientRowKey(patient);
  ...
  const savePatientRow = useCallback(
    async (patient: PatientRow, draft?: PatientDraft) => {
      const rowKey = getPatientRowKey(patient);
        ...
        const selectedDoctor =
          activeDoctors.find(
            (doctor) =>
              String(doctor.name ?? "")
                .trim()
                .toLocaleLowerCase("ar") === normalizedDoctorName,
          ) ?? null;

        if (nextDoctor && !selectedDoctor) {
          throw new Error(
            "Doctor name must match an active doctor from directory",
          );
        }

        const selectedDoctorCode =
          String(selectedDoctor?.code ?? "").trim() || null;
        const selectedDoctorId =
          String(selectedDoctor?.id ?? "").trim() || null;
        const selectedLocationType = selectedDoctor
          ? selectedDoctor.locationType === "external"
            ? "external"
            : "center"
          : null;
        const doctorUpdates =
              ...
              ? {
                  serviceSheetTypeByCode: {
                    ...
                    typeof existingData.serviceSheetTypeByCode === "object"
                      ? (existingData.serviceSheetTypeByCode as Record<
                          string,
                          unknown
                        >)
                      : {}),
  ...
  const isManualLockEnabled = useCallback(
    (patient: PatientRow) => {
      if (
        Object.prototype.hasOwnProperty.call(manualLockOverrides, patient.id)
      ) {
        return Boolean(manualLockOverrides[patient.id]);
      }
      return (
        Boolean(patient.syncLockManual) ||
        String(patient.manualEditedAt ?? "").trim().length > 0
      );
    },
    [manualLockOverrides],
  );
  ...
  const handleToggleManualLock = useCallback(
    async (patient: PatientRow) => {
      ...
      try {
        ...
        toast.success(
          nextEnabled ? "Manual lock enabled" : "Manual lock disabled",
        );
        await utils.medical.getAllPatients.invalidate();
      } catch (error) {
  ...
  const handleDeleteFromMssql = useCallback(
    async (patient: PatientRow) => {
      ...
      const confirmed = window.confirm(
        `Delete patient ${patientCode} from MSSQL only?`,
      );
      if (!confirmed) return;
      try {
        ...
        if ((result as { deleted?: boolean }).deleted)
          toast.success(`Deleted ${patientCode} from MSSQL`);
        else toast.info(`No MSSQL row found for ${patientCode}`);
      } catch (error) {
        toast.error(
          getTrpcErrorMessage(error, "Failed to delete patient from MSSQL"),
        );
      }
    },
  ...
  const handleDeletePatient = useCallback(
    async (patient: PatientRow) => {
      ...
      const confirmName = window.prompt(
        `Type the patient name \"${displayName}\" to confirm deletion:`,
      );
  ...
  const allVisibleSelected =
    ...
    visiblePatients.every((patient) => selectedPatients.has(patient.id));
  ...
  const toggleSelectedPatient = useCallback(
