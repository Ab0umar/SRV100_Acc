...
export function FollowupTab({
  examinations, visitsData, surgeries, followups, followupSheets, isAdmin, patientId,
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "exam" | "followup">("all");
  ...
  const followupEntries = (followupSheets ?? []).flatMap((sheet: any) =>
    (sheet.items ?? [])
      ...
      .map((item: any) => {
        ...
        return { _type: "followup" as const, _date: new Date(item.followupDate), _sheetType: sheet.sheetType, ...item, refOD, refOS };
      })
  );

  // Merge exams + followup items, sort newest first
  const allEntries = [
    ...examinations.map((e: any) => ({ _type: "exam" as const, _date: new Date(visitsData.find((v: any) => v.id === e.visitId)?.visitDate || e.createdAt), ...e })),
    ...followupEntries,
    ...
    <div className="space-y-6">
      ...
      <Card className="border-border/80 bg-background/92 shadow-sm">
        <CardHeader className="border-b border-border flex items-center justify-between">
          ...
          {patientId && (
            <Button size="sm" variant="outline" className="gap-2" onClick={() => setLocation(`/followup/${patientId}`)}>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-2 pb-1">
            <span className="text-xs font-medium text-muted-foreground">عرض:</span>
            <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v as "all" | "exam" | "followup")}>
            ...
            <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="h-7 w-32 text-xs" dir="ltr" />
            <span className="text-xs text-muted-foreground">إلى:</span>
            <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="h-7 w-32 text-xs" dir="ltr" />
            {(typeFilter !== "all" || dateFrom || dateTo) && (
              <button type="button" onClick={() => { setTypeFilter("all"); setDateFrom(""); setDateTo(""); }} className="text-xs text-muted-foreground hover:text-foreground">
                مسح
              </button>
            )}
          </div>

          {filteredEntries.length === 0 && <p className="text-sm text-muted-foreground">لا توجد نتائج</p>}
          {filteredEntries.map((entry: any, idx: number) => {
            const isExam = entry._type === "exam";
            const isEditing = isExam && editingVisitId === entry.visitId;
            return (
              <div key={`${entry._type}-${entry.id}-${idx}`} className="rounded-2xl border border-border bg-muted/70 p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isExam ? "bg-primary/10 text-primary" : "bg-emerald-500/10 text-emerald-600"}`}>
                        {isExam ? "فحص" : "متابعة"}
                      </span>
                      {!isExam && entry._sheetType && (
                        <span className="text-xs text-muted-foreground">({entry._sheetType})</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {isEditing && isAdmin ? (
                      <>
                        ...
                        <Button size="sm" variant="ghost" onClick={() => updateVisitDateMutation.mutate({ visitId: entry.visitId, visitDate: editVisitDate })} disabled={updateVisitDateMutation.isPending}>
                      ...
                      <>
                        <span className="text-xs text-muted-foreground">{formatDate(entry._date)}</span>
                        {isExam && isAdmin && (
                          <Button size="sm" variant="ghost" onClick={() => { const d = new Date(entry._date).toISOString().split("T")[0]; setEditVisitDate(d); setEditingVisitId(entry.visitId); }} className="text-card-foreground hover:bg-primary/5">
                    ...
                    {isExam && isAdmin && (
                      <Button variant="ghost" size="sm" aria-label="حذف الزيارة" disabled={deleteVisitMutation.isPending || !entry.visitId} className="h-11 w-11 p-0"
                        onClick={async () => {
                          if (!entry.visitId) { toast.error("خطأ: لا يوجد رقم زيارة"); return; }
                          if (confirm(`هل أنت متأكد من حذف الزيارة رقم ${entry.visitId} (${formatDate(entry.createdAt)}) وكل بياناتها فقط؟`)) {
                            try {
                              if (!entry.id) { toast.error("خطأ: معرّف الفحص مفقود"); return; }
                              await deleteExamMutation.mutateAsync({ examinationId: entry.id });
                ...
                {/* Exam fields */}
                {isExam && (entry.ucvaOD || entry.ucvaOS || entry.bcvaOD || entry.bcvaOS) && (
                  <div className="text-sm pt-2 border-t border-border">
                    <span className="font-semibold text-foreground">الحدة البصرية:</span>
                    <p className="text-muted-foreground mt-1">
                      {entry.ucvaOD && `UCVA OD: ${entry.ucvaOD}`}{entry.ucvaOD && entry.ucvaOS && " | "}{entry.ucvaOS && `UCVA OS: ${entry.ucvaOS}`}
                    </p>
                    {(entry.bcvaOD || entry.bcvaOS) && (
                      <p className="text-muted-foreground">{entry.bcvaOD && `BCVA OD: ${entry.bcvaOD}`}{entry.bcvaOD && entry.bcvaOS && " | "}{entry.bcvaOS && `BCVA OS: ${entry.bcvaOS}`}</p>
                    )}
                  </div>
                )}
                {isExam && (entry.sphereOD || entry.sphereOS || entry.cylinderOD || entry.cylinderOS) && (
                  <div className="text-sm pt-2 border-t border-border">
                    <span className="font-semibold text-foreground">الانكسار:</span>
                    {entry.sphereOD && <p className="text-muted-foreground mt-1">OD: {entry.sphereOD} / {entry.cylinderOD || "-"} x {entry.axisOD || "-"}</p>}
                    {entry.sphereOS && <p className="text-muted-foreground">OS: {entry.sphereOS} / {entry.cylinderOS || "-"} x {entry.axisOS || "-"}</p>}
                  </div>
                )}
                {isExam && (entry.iopOD || entry.iopOS) && (
                  <div className="text-sm pt-2 border-t border-border">
                    <span className="font-semibold text-foreground">ضغط العين:</span>
                    <p className="text-muted-foreground mt-1">{entry.iopOD && `OD: ${entry.iopOD}`}{entry.iopOD && entry.iopOS && " | "}{entry.iopOS && `OS: ${entry.iopOS}`}</p>
                  </div>
                )}
                ...
                {!isExam && (entry.vaOD || entry.vaOS) && (
                  <div className="text-sm pt-2 border-t border-border">
                    <span className="font-semibold text-foreground">الحدة البصرية:</span>
                    <p className="text-muted-foreground mt-1">OD: {entry.vaOD || "-"} | OS: {entry.vaOS || "-"}</p>
                  </div>
                )}
                {!isExam && (() => {
                  const hasRef = (s: any) => s && s !== "----";
                  const odS = hasRef(entry.refOD?.s); const osS = hasRef(entry.refOS?.s);
                  const v = (val: any) => (val && val !== "----") ? val : "-";
                  if (!odS && !osS) return null;
                  return (
                    <div className="text-sm pt-2 border-t border-border">
                      <span className="font-semibold text-foreground">الانكسار:</span>
                      {odS && <p className="text-muted-foreground mt-1">OD: {v(entry.refOD.s)} / {v(entry.refOD.c)} x {v(entry.refOD.axis ?? entry.refOD.a)}</p>}
                      {osS && <p className="text-muted-foreground">OS: {v(entry.refOS.s)} / {v(entry.refOS.c)} x {v(entry.refOS.axis ?? entry.refOS.a)}</p>}
                    </div>
                  );
                })()}
                {!isExam && (entry.iopOD || entry.iopOS) && (
                ...
                {!isExam && entry.treatment && (
                  <div className="text-sm pt-2 border-t border-border">
                    <span className="font-semibold text-foreground">العلاج:</span>
                    <p className="text-muted-foreground mt-1">{entry.treatment}</p>
                  </div>
                )}
              </div>
            );
      ...
      {followupSheets.length > 0 && (
        <Card className="border-border/80 bg-background/92 shadow-sm">
          <CardHeader className="border-b border-border"><CardTitle>ملاحظات المتابعة</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {followupSheets.map((sheet: any) => (
              <div key={sheet.id} className="rounded-2xl border border-border bg-muted/70 p-4 space-y-3">
                <div className="flex items-center justify-between border-b pb-2">
                  <span className="font-semibold">ملف متابعة #{sheet.id}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-muted-foreground">{sheet.sheetType}</span>
                    <span className="text-xs text-muted-foreground">إصدار {sheet.version}</span>
                  </div>
                </div>
                {Array.isArray(sheet.items) && sheet.items.length > 0 ? (
                  <div className="space-y-2">
                    {sheet.items.map((item: any, idx: number) => (
                      <div key={item.id || idx} className="border-l-2 border-primary/50 pl-3 py-1 text-sm">
                        <div className="flex items-center justify-between">
                          <span className="font-medium">{item.followupName || `متابعة ${idx + 1}`}</span>
                          <span className="text-xs text-muted-foreground">{formatDate(item.followupDate)}</span>
                        </div>
                        {(item.vaOD || item.vaOS) && (
                          <p className="text-muted-foreground text-xs mt-1">
                            الحدة: {item.vaOD && `OD: ${item.vaOD}`}{item.vaOD && item.vaOS && " | "}{item.vaOS && `OS: ${item.vaOS}`}
                          </p>
                        )}
                        {(item.sphereOD || item.sphereOS) && (
                          <p className="text-muted-foreground text-xs">
                            الانكسار: {item.sphereOD && `OD: ${item.sphereOD}/${item.cylinderOD || "-"}`}{item.sphereOD && item.sphereOS && " | "}{item.sphereOS && `OS: ${item.sphereOS}/${item.cylinderOS || "-"}`}
                          </p>
                        )}
                        {(item.iopOD || item.iopOS) && (
                          <p className="text-muted-foreground text-xs">
                            ضغط العين: {item.iopOD && `OD: ${item.iopOD}`}{item.iopOD && item.iopOS && " | "}{item.iopOS && `OS: ${item.iopOS}`}
                          </p>
                        )}
                        {item.notes && (
                          <p className="text-muted-foreground text-xs mt-1">ملاحظات: {item.notes}</p>
                        )}
                      </div>
                    ))}
                  </div>
