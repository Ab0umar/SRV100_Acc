...
export function BookingShortcutModule({
  onAddBooking,
}: BookingShortcutModuleProps) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const list of bookingLists) {
      ...
      const total = items.reduce(
        (acc: number, item: any) => acc + (item.casesCount || 1),
        0,
      );
      groups[d] = {
        doctor: d,
        ...
        <table className={reportStyles.gridTable}>
          <thead>
            <tr>
              ...
              <th className="font-bold text-center w-16 text-[11px] py-1 px-2">
                العدد
              </th>
            </tr>
          </thead>
          <tbody>
            {bookingsQuery.isLoading ? (
              ...
              <tr>
                <td
                  colSpan={2}
                  className="text-center py-4 text-muted-foreground text-[10px] italic"
                >
              ...
              grouped.map((group) => (
                ...
                >
                  ...
                  <td className="text-center font-bold tabular-nums text-xs py-1 px-2">
                    {group.count}
                  </td>
                </tr>
              ))
          ...
          {grouped.length > 0 && (
            <tfoot>
              <tr className={reportStyles.grandTotalRow}>
                ...
                <td className="text-center font-bold text-[11px] py-1 px-2 tabular-nums">
                  {totalCases}
                </td>
              </tr>
            </tfoot>
