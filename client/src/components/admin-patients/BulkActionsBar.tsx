...
export function BulkActionsBar({
  ...
  return (
    <div
      className="overflow-x-auto rounded-xl border border-dashed border-border/80 bg-muted/10 px-3 py-3"
      dir="rtl"
    >
      <div className="flex min-w-0 flex-wrap items-center justify-end gap-2">
          ...
          <SelectContent>
            ...
            {activeDoctors.map((doctor) => (
              <SelectItem key={doctor.id} value={doctor.id}>
                {doctor.name} (
                {doctor.locationType === "external" ? "خارجي" : "مركز"})
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        <Button
          ...
          onClick={onSetFilteredDoctor}
          disabled={isBusy || isSaveStatePending}
        >
          تعيين للمرشحين
        </Button>
        ...
        <Select
          value={bulkSheetType}
          onValueChange={(value) =>
            onBulkSheetTypeChange(value as "none" | SheetTypeChoice)
          }
        >
        ...
        <Button
          variant="outline"
          className="rounded-lg"
          onClick={onSetFilteredSheetType}
          disabled={isBusy}
        >
          تعيين للمرشحين
        </Button>
        ...
        <Select
          value={bulkManualLock}
          onValueChange={(value) =>
            onBulkManualLockChange(value as "none" | "on" | "off")
          }
        >
        ...
        <Button
          variant="outline"
          className="rounded-lg"
          onClick={onSetFilteredManualLock}
          disabled={isSaveStatePending}
        >
          تعيين للمرشحين
        </Button>
        ...
        <Button
          variant="outline"
          className="rounded-lg"
          onClick={onUndoLastBulkAction}
          disabled={isUndoPending || !canUndo}
        >
          تراجع
        </Button>
      </div>
