...
function AdminPatientTransactions({
  patientId,
  serviceCodeToLabel,
}: {
  patientId: number;
  ...
  if (entriesQuery.isLoading)
    return <div className="text-xs text-muted-foreground">جاري التحميل…</div>;
  if (rows.length === 0)
    return <div className="text-xs text-muted-foreground">لا توجد تعاملات</div>;
  ...
  return (
    <div className="space-y-1 text-right text-xs" dir="rtl">
      {rows.map((entry) => {
        ...
        const code = String(rawEntry.serviceCode ?? "")
          .trim()
          .toLowerCase();
        const name = String(
          serviceCodeToLabel.get(code) ?? rawEntry.serviceName ?? code ?? "-",
        ).trim();
...
const arePatientItemPropsEqual = (
  prev: AdminPatientItemProps,
  next: AdminPatientItemProps,
) =>
...
const AdminPatientCard = memo(function AdminPatientCard({
  ...
  const rowServiceCode = String(
    (patient as any).__serviceCodeSingle ?? patient.serviceCode ?? "",
  )
    .trim()
    .toLowerCase();
  ...
  return (
    <div
      className={cn(
        ...
        isUnsavedRow ? "border-warning/70 bg-warning/10/70" : undefined,
      )}
    >
      <div className="flex items-start gap-2">
        <Checkbox
          ...
          onCheckedChange={(checked) =>
            onToggleSelectedPatient(patient.id, Boolean(checked))
          }
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          ...
          <Input
            ...
            onChange={(event) =>
              onSetDraftField(patient, "fullName", event.target.value)
            }
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-x-2 gap-y-2 rounded-xl border border-border/60 bg-muted/40 px-3 py-2 text-xs">
        ...
        <div className="text-foreground">
          <Input
            ...
            onChange={(event) =>
              onSetDraftField(patient, "treatingDoctor", event.target.value)
            }
        ...
        <div className="text-foreground">
          <Select
            ...
            onValueChange={(value) =>
              onSetDraftField(patient, "serviceType", value)
            }
          >
      ...
      <div className="mt-2.5 flex items-center justify-between gap-2">
        <Button
          ...
          className={
            manualLockEnabled
              ? "rounded-lg border-secondary/50 bg-secondary text-xs font-bold text-secondary-foreground shadow-sm hover:bg-secondary/80"
              : "rounded-lg border-warning/50 bg-warning/20 text-xs font-bold text-warning hover:bg-warning/30"
          }
        ...
        <div className="flex items-center gap-1.5">
          <Button
            variant="outline"
            size="sm"
            className="gap-1.5 rounded-lg border-primary/30 bg-primary text-primary-foreground hover:bg-primary/90"
  ...
  return (
    <Fragment>
      <TableRow
        className={cn(
          "border-border/70",
          isUnsavedRow ? "bg-warning/10/70" : undefined,
        )}
      >
        ...
        <TableCell>
          <Button
            ...
            className={
              manualLockEnabled
                ? "rounded-lg border-secondary/50 bg-secondary font-bold text-secondary-foreground shadow-sm hover:bg-secondary/80"
                : "rounded-lg border-warning/50 bg-warning/20 font-bold text-warning hover:bg-warning/30"
            }
            onClick={() => onToggleManualLock(patient)}
            disabled={savePatientPageStatePending}
          >
        ...
        <TableCell>
          <div className="flex flex-col items-end gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <Button
                variant="outline"
                className="gap-2 rounded-lg border-primary/30 bg-primary text-primary-foreground hover:bg-primary/90"
                disabled={updatePatientPending}
                onClick={() => onSavePatientRow(patient)}
              >
                {status?.state === "saving" ? (
...
function AdminPatientsTableComponent({
  ...
  const mobileCardsTotalPages = Math.ceil(
    visiblePatients.length / MOBILE_CARDS_PER_PAGE,
  );
  const mobileCardsStartIdx = (mobileCardPage - 1) * MOBILE_CARDS_PER_PAGE;
  const mobileCardsEndIdx = Math.min(
    mobileCardsStartIdx + MOBILE_CARDS_PER_PAGE,
    visiblePatients.length,
  );
  const paginatedMobileCards = visiblePatients.slice(
    mobileCardsStartIdx,
    mobileCardsEndIdx,
  );
  ...
  return (
    <>
          ...
          {visiblePatients.length > 0 && (
            ...
            >
              <div className="text-sm tabular-nums text-muted-foreground">
                صفحة {mobileCardPage} من {mobileCardsTotalPages}
              </div>
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    setMobileCardPage(Math.max(1, mobileCardPage - 1))
                  }
                ...
                <Button
                  ...
                  onClick={() =>
                    setMobileCardPage(
                      Math.min(mobileCardsTotalPages, mobileCardPage + 1),
                    )
                  }
              ...
              <TableRow className="sticky top-0 z-10 border-b bg-muted/40 hover:bg-muted/40">
                <TableHead className="w-10 py-4 text-center">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      onToggleSelectAllVisible(Boolean(checked))
                    }
                  />
                </TableHead>
                <TableHead className="py-4 font-semibold">الكود</TableHead>
                <TableHead className="min-w-[200px] py-4 font-semibold">
                  الاسم
                </TableHead>
                <TableHead className="min-w-[200px] py-4 font-semibold">
                  الطبيب
                </TableHead>
                <TableHead className="min-w-[140px] py-4 font-semibold">
                  النوع من الكود
                </TableHead>
                <TableHead className="min-w-[170px] py-4 font-semibold">
                  نوع الخدمة
                </TableHead>
                <TableHead className="py-4 font-semibold">القفل</TableHead>
                <TableHead className="min-w-[160px] py-4 font-semibold">
                  الإجراءات
                </TableHead>
              </TableRow>
            </TableHeader>
      ...
      <div
        className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border/60 bg-muted/10 px-3 py-2"
        dir="rtl"
      >
        <div className="text-sm tabular-nums text-muted-foreground">
          صفحة {currentPage}
