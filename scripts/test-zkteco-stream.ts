  ...
  return new Promise<void>((resolve) => {
    ...
    const timeoutHandle = setTimeout(() => {
      if (!dataReceived) {
        ...
        console.log(
          `   Device may not be in streaming mode or may need initialization\n`,
        );
      }
      socket.destroy();
      resolve();
    ...
    socket.on("data", (data: Buffer) => {
      ...
      if (data.length >= 10) {
        ...
        if (data.length >= 8) {
          ...
          console.log(
            `    Timestamp (Unix): ${timestamp} (${new Date(timestamp * 1000).toISOString()})`,
          );
        }
        if (data.length >= 9) {
          const direction = data[8];
          console.log(
            `    Direction: ${direction === 1 ? "IN" : direction === 0 ? "OUT" : "UNKNOWN"}`,
          );
        }
      }
    ...
    socket.on("close", () => {
      ...
      if (punchCount === 0 && dataReceived === false) {
        ...
        console.log(`\n[POSSIBLE SOLUTIONS]`);
        console.log(
          `1. Taratus.exe may have special lock/protocol only it knows`,
        );
        console.log(
          `2. Device may require Access DB sync approach (Phase 1 fallback)`,
        );
        console.log(
          `3. Device may need different initialization/handshake sequence`,
        );
        console.log(
          `4. Check device documentation for actual protocol details\n`,
        );
      }
      resolve();
    });
