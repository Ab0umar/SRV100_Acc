...
async function main() {
  ...
  try {
    console.log("Step 1: MSSQL deletion requires manual confirmation (0000807)\n");
    ...
    const checkResult = await pool.request()
      .input('code', sql.VarChar, '0000807')
      .query(`SELECT COUNT(*) as count FROM op2026.dbo.PAPATMF WHERE PAT_CD = @code`);

    console.log(`⚠ Patient 0000807 would be deleted from MSSQL (NOT DELETING - manual confirmation required)\n`);

    console.log("Step 2: Checking current MySQL patients (no deletion)...\n");

    // Check patient count instead of deleting
    const [existingPatients] = await mysqlConn.query(`SELECT COUNT(*) as count FROM patients`) as any[];
    console.log(`⚠ Existing patients found: ${existingPatients[0].count} (NOT DELETING - manual confirmation required)\n`);
    ...
    console.log("Step 3: Resyncing patients from MSSQL...\n");
    ...
    console.log(`\n✓ COMPLETE! MSSQL patient deleted and MySQL resynced`);
  } finally {
    await mysqlConn.end();
  }
}
