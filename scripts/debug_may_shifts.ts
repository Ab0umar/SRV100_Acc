...
async function check() {
  ...
  att.forEach((r: any) =>
    console.log(
      `  staff_id ${r.staff_id}: scheduled=${r.scheduled}, attended=${r.attended}`,
    ),
  );
  ...
  punches.forEach((r: any) =>
    console.log(`  emp_cd ${r.emp_cd}: ${r.cnt} ${r.status} records`),
  );

  if (punches.length === 0) {
    console.log(
      "  (NONE - all employees marked as 'absent' in attendance_daily)",
    );
  }
