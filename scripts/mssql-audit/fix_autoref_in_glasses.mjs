...
for (const r of rows) {
  ...
  backup.push({
    ...
    fullName: r.fullName,
    before: r.glassesData,
  });

  const next = JSON.parse(JSON.stringify(g));
  if (odMatch) {
    next.od = { ...next.od, s: "", c: "", axis: "" };
  }
  if (osMatch) {
    next.os = { ...next.os, s: "", c: "", axis: "" };
  }

  await conn.execute(`UPDATE examinations SET glassesData = ? WHERE id = ?`, [
    JSON.stringify(next),
    r.examId,
  ]);
  updated++;
...
const backupPath = path.join(
  ...
  "scripts",
  "mssql-audit",
  `glassesData-backup-${Date.now()}.json`,
);
fs.writeFileSync(backupPath, JSON.stringify(backup, null, 2), "utf8");

console.log(`Updated ${updated} examination rows.`);
console.log(`Backup of original glassesData written to: ${backupPath}`);
