...
async function main() {
  ...
  try {
    ...
    // Get all doctors from MDTEAM by CODE
    ...
    console.log(`Found ${doctors.length} active doctors in MDTEAM\n`);
    ...
    // Get all services from SRVCMF by SRV_CD
    ...
    console.log(`Found ${services.length} active services in SRVCMF\n`);
    ...
    for (const ps of patientServices) {
      ...
      // Update patient with doctorId as UUID and treatingDoctor as name
      try {
        await mysqlConn.query(
          `UPDATE patients SET serviceType = ?, doctorId = ?, treatingDoctor = ? WHERE id = ?`,
          [
            mappedServiceType,
            doctorUUID,
            doctorName || null,
    ...
    const [stats] = await mysqlConn.query(`
      SELECT
        COUNT(*) as total,
        SUM(CASE WHEN treatingDoctor IS NOT NULL AND treatingDoctor != '' THEN 1 ELSE 0 END) as with_doctor,
    ...
    console.log(`Patients with doctor: ${stats[0].with_doctor}`);
    console.log(`Patients with service: ${stats[0].with_service}`);
