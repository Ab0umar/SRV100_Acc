...
exports.APP_NOTIFICATION_SETTINGS_KEY = exports.APP_NOTIFICATION_FEED_KEY =
  void 0;
...
const normalizeFeed = (value) => {
  ...
  for (const item of value) {
    ...
    const kindRaw = String(row.kind ?? "info")
      .trim()
      .toLowerCase();
    const kind =
      kindRaw === "success" || kindRaw === "warning" || kindRaw === "error"
        ? kindRaw
        : "info";
    const targetRolesRaw = Array.isArray(row.targetRoles)
      ? row.targetRoles
      : [];
    const targetRoles = targetRolesRaw
      ...
      .filter(Boolean);
    const targetUserIdsRaw = Array.isArray(row.targetUserIds)
      ? row.targetUserIds
      : [];
    ...
    normalized.push({
      ...
      entityId: Number.isFinite(Number(row.entityId))
        ? Number(row.entityId)
        : null,
...
async function pushAppNotification(input) {
  const row = await db
    .getSystemSetting(exports.APP_NOTIFICATION_FEED_KEY)
    .catch(() => null);
  let existingFeed = [];
  if (row?.value) {
    try {
  ...
  const nextFeed = [entry, ...existingFeed].slice(
    0,
...
async function getAppNotificationSettings() {
  const row = await db
    .getSystemSetting(exports.APP_NOTIFICATION_SETTINGS_KEY)
    .catch(() => null);
  if (!row?.value) return DEFAULT_APP_NOTIFICATION_SETTINGS;
  try {
    const parsed = JSON.parse(String(row.value));
    const operationsPushUserIdsRaw = Array.isArray(parsed.operationsPushUserIds)
      ? parsed.operationsPushUserIds
      : [];
    ...
    return {
      ...
      operationsPushUserIds:
        operationsPushUserIds.length > 0
          ? operationsPushUserIds
          : DEFAULT_APP_NOTIFICATION_SETTINGS.operationsPushUserIds,
    };
  } catch {
