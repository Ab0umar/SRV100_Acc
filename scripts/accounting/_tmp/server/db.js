...
exports.normalizePermissionPathsForTeamMirror =
  normalizePermissionPathsForTeamMirror;
...
exports.getActivePushDeviceRegistrationsByUser =
  getActivePushDeviceRegistrationsByUser;
...
async function getUserByUsername(username) {
  ...
  const result = await db
    ...
    .where(
      (0, drizzle_orm_1.or)(
        (0, drizzle_orm_1.eq)(schema_1.users.username, normalized),
        (0, drizzle_orm_1.eq)(schema_1.users.username, legacy),
      ),
    )
    .limit(1);
...
async function stagePatientImportRows(batchId, rows) {
  ...
  await db
    .delete(schema_1.patientImportStaging)
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.patientImportStaging.batchId,
        normalizedBatchId,
      ),
    );
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        ...
        (0, drizzle_orm_1.eq)(schema_1.patientImportStaging.status, "invalid"),
      ),
    )
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        ...
        (0, drizzle_orm_1.eq)(schema_1.patientImportStaging.status, "valid"),
      ),
    )
    .orderBy(schema_1.patientImportStaging.rowNumber);
  const directory = await getDoctorDirectoryCached();
    ...
    try {
      ...
      if (!patientCode || !fullName) {
        ...
        await db
          ...
          .where(
            (0, drizzle_orm_1.eq)(schema_1.patientImportStaging.id, row.id),
          );
        continue;
      }
      const payload = {
      ...
      if (existing) {
        await db
          ...
          .set(payload)
          .where(
            (0, drizzle_orm_1.eq)(schema_1.patients.id, Number(existing.id)),
          );
        updated += 1;
      } else {
      ...
      if (doctorName) {
        ...
        if (savedPatient?.id) {
          const existingState = await getPatientPageState(
            savedPatient.id,
            "examination",
          );
          const existingData =
...
async function getNextPatientCode() {
  ...
  const rows = await db
    .select({
      maxCode: (0,
      drizzle_orm_1.sql)`MAX(CASE WHEN ${schema_1.patients.patientCode} REGEXP '^[0-9]{4}$' THEN CAST(${schema_1.patients.patientCode} AS UNSIGNED) ELSE NULL END)`,
    })
    .from(schema_1.patients);
...
async function getPatientByCode(patientCode) {
  ...
  const result = await db
    .select()
    .from(schema_1.patients)
    .where((0, drizzle_orm_1.eq)(schema_1.patients.patientCode, patientCode))
    .limit(1);
  return result.length > 0 ? decodePatientRow(result[0]) : null;
}
async function searchPatients(searchTerm, sheetType) {
  ...
  const buildTokenClause = (token) => {
    ...
    return (0, drizzle_orm_1.or)(
      (0, drizzle_orm_1.like)(schema_1.patients.fullName, t),
      (0, drizzle_orm_1.like)(schema_1.patients.fullName, lt),
      (0, drizzle_orm_1.like)(schema_1.patients.patientCode, t),
      (0, drizzle_orm_1.like)(schema_1.patients.phone, t),
      (0, drizzle_orm_1.like)(schema_1.patients.alternatePhone, t),
    );
  };
  const phraseClause = (0, drizzle_orm_1.or)(
    (0, drizzle_orm_1.like)(schema_1.patients.fullName, term),
    (0, drizzle_orm_1.like)(schema_1.patients.fullName, legacyTerm),
    (0, drizzle_orm_1.like)(schema_1.patients.patientCode, term),
    (0, drizzle_orm_1.like)(schema_1.patients.phone, term),
    (0, drizzle_orm_1.like)(schema_1.patients.alternatePhone, term),
  );
  ...
  if (sheetType) {
    ...
    whereClause = (0, drizzle_orm_1.and)(
      textMatch,
      (0, drizzle_orm_1.inArray)(schema_1.patients.id, patientIds),
    );
  }
  const result = await db
    .select()
...
async function deletePatientWithAllData(patientId) {
  ...
  const patientVisits = await db
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.visits.patientId, patientId));
  const visitIds = patientVisits.map((v) => v.id);
  if (visitIds.length > 0) {
    // Delete data related to visits
    await db
      .delete(schema_1.testRequestItems)
      .where(
        (0, drizzle_orm_1.inArray)(
          schema_1.testRequestItems.testRequestId,
          db
            .select({ id: schema_1.testRequests.id })
            .from(schema_1.testRequests)
            .where(
              (0, drizzle_orm_1.inArray)(
                schema_1.testRequests.visitId,
                visitIds,
              ),
            ),
        ),
    ...
    await db
      .delete(schema_1.examinations)
      .where(
        (0, drizzle_orm_1.inArray)(schema_1.examinations.visitId, visitIds),
      );
    await db
      .delete(schema_1.pentacamResults)
      .where(
        (0, drizzle_orm_1.inArray)(schema_1.pentacamResults.visitId, visitIds),
      );
    await db
      .delete(schema_1.doctorReports)
      .where(
        (0, drizzle_orm_1.inArray)(schema_1.doctorReports.visitId, visitIds),
      );
  }
    ...
    .where(
      (0, drizzle_orm_1.inArray)(
        ...
        db
          ...
          .from(schema_1.testRequests)
          .where(
            (0, drizzle_orm_1.eq)(schema_1.testRequests.patientId, patientId),
          ),
      ),
    )
    .catch(() => {});
  ...
  await db
    .delete(schema_1.testRequests)
    .where((0, drizzle_orm_1.eq)(schema_1.testRequests.patientId, patientId));
  // Delete prescription items before prescriptions
  await db
    .delete(schema_1.prescriptionItems)
    .where(
      (0, drizzle_orm_1.inArray)(
        schema_1.prescriptionItems.prescriptionId,
        db
          .select({ id: schema_1.prescriptions.id })
          .from(schema_1.prescriptions)
          .where(
            (0, drizzle_orm_1.eq)(schema_1.prescriptions.patientId, patientId),
          ),
      ),
    )
    .catch(() => {});
  await db
    .delete(schema_1.prescriptions)
  ...
  await db
    .delete(schema_1.surgeries)
    .where((0, drizzle_orm_1.eq)(schema_1.surgeries.patientId, patientId));
  // Delete post-op followups and consent forms
  await db
    .delete(schema_1.postOpFollowups)
    .where(
      (0, drizzle_orm_1.eq)(schema_1.postOpFollowups.patientId, patientId),
    );
  await db
    .delete(schema_1.consentForms)
    .where((0, drizzle_orm_1.eq)(schema_1.consentForms.patientId, patientId));
  await db
    .delete(schema_1.medicalHistoryChecklist)
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.medicalHistoryChecklist.patientId,
        patientId,
      ),
    );
  await db
    .delete(schema_1.sheetEntries)
    .where((0, drizzle_orm_1.eq)(schema_1.sheetEntries.patientId, patientId));
  await db
    .delete(schema_1.patientServiceEntries)
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.patientServiceEntries.patientId,
        patientId,
      ),
    );
  // Delete visits (but NOT the patient record itself)
  await db
    .delete(schema_1.visits)
    .where((0, drizzle_orm_1.eq)(schema_1.visits.patientId, patientId));
...
async function deleteVisitWithAllData(visitId) {
  ...
  const visitTestRequests = await db
    .select({ id: schema_1.testRequests.id })
    .from(schema_1.testRequests)
    .where((0, drizzle_orm_1.eq)(schema_1.testRequests.visitId, visitId));
  ...
  await db
    .delete(schema_1.examinations)
    .where((0, drizzle_orm_1.eq)(schema_1.examinations.visitId, visitId));
  await db
    .delete(schema_1.pentacamResults)
    .where((0, drizzle_orm_1.eq)(schema_1.pentacamResults.visitId, visitId));
  await db
    .delete(schema_1.doctorReports)
    .where((0, drizzle_orm_1.eq)(schema_1.doctorReports.visitId, visitId));
...
async function deleteExaminationDirect(examinationId) {
  ...
  const examination = await db
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.examinations.id, examinationId))
    .limit(1);
...
async function fixOrphanedExaminations() {
  ...
  const orphanedExams = await db
    ...
    .where(
      (0, drizzle_orm_1.or)(
        (0, drizzle_orm_1.eq)(schema_1.examinations.visitId, 0),
        (0, drizzle_orm_1.eq)(schema_1.examinations.visitId, null),
      ),
    );
  let fixedCount = 0;
...
async function checkInvalidVisitIds() {
  ...
  const visitsWithId0 = await db
    .select()
    .from(schema_1.visits)
    .where((0, drizzle_orm_1.eq)(schema_1.visits.id, 0));
  ...
  const allExams = await db
    .select({ visitId: schema_1.examinations.visitId })
    .from(schema_1.examinations);
  const allVisitIds = await db
    .select({ id: schema_1.visits.id })
    .from(schema_1.visits);
...
async function fixExamsWithVisitId0() {
  ...
  const examsWithId0 = await db
    ...
    .from(schema_1.examinations)
    .where((0, drizzle_orm_1.eq)(schema_1.examinations.visitId, 0));
  ...
  const pentacamData = await db
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.pentacamResults.visitId, 0))
    .limit(1000);
  ...
  const reportData = await db
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.doctorReports.visitId, 0))
    .limit(1000);
  for (const report of reportData) {
    const visitId = patientVisits.get(report.patientId);
    if (visitId) {
      await db
        .update(schema_1.doctorReports)
        .set({ visitId })
...
async function fixVisitsWithoutAppointmentId() {
  ...
  const visitsWithoutAppointment = await db
    ...
    .from(schema_1.visits)
    .where((0, drizzle_orm_1.isNull)(schema_1.visits.appointmentId))
...
async function checkVisitsWithoutAppointments() {
  ...
  for (const visit of visitsWithoutAppointment) {
    const patientAppointments = await db
      ...
      .where(
        (0, drizzle_orm_1.eq)(schema_1.appointments.patientId, visit.patientId),
      );
    results.push({
      visitId: visit.id,
...
function buildPatientFilterClauses(filters) {
  ...
  if (normalizedBranch) {
    whereClauses.push(
      (0, drizzle_orm_1.eq)(schema_1.patients.branch, normalizedBranch),
    );
  }
  ...
  if (normalizedDateFrom) {
    whereClauses.push(
      (0, drizzle_orm_1.gte)(schema_1.patients.lastVisit, normalizedDateFrom),
    );
  }
  const normalizedDateTo = String(filters?.dateTo ?? "").trim();
  if (normalizedDateTo) {
    whereClauses.push(
      (0, drizzle_orm_1.lte)(schema_1.patients.lastVisit, normalizedDateTo),
    );
  }
  ...
  if (normalizedLocationType) {
    whereClauses.push(
      (0, drizzle_orm_1.eq)(
        schema_1.patients.locationType,
...
async function getAllPatients(options) {
  ...
  let query = db
    ...
    .orderBy(
      (0,
      drizzle_orm_1.sql)`CAST(${schema_1.patients.patientCode} AS UNSIGNED) ASC, ${schema_1.patients.patientCode} ASC`,
    )
    .limit(limitValue + 1);
  if (whereExpr) {
...
async function getPatientStats(year, month, filters) {
  ...
  const buildServiceCodeMatchExpr = (serviceType) => {
    ...
    if (!codes.length) return (0, drizzle_orm_1.sql)`0 = 1`;
    return (0,
    drizzle_orm_1.sql)`LOWER(TRIM(${schema_1.patientServiceEntries.serviceCode})) IN (${drizzle_orm_1.sql.join(
      codes.map((code) => (0, drizzle_orm_1.sql)`${code}`),
      (0, drizzle_orm_1.sql)`, `,
    )})`;
  };
  const effectiveServiceDate = (0,
  drizzle_orm_1.sql)`COALESCE(${schema_1.patientServiceEntries.serviceDate}, DATE(${schema_1.patientServiceEntries.updatedAt}))`;
  const whereClauses = [
    (0, drizzle_orm_1.sql)`YEAR(${effectiveServiceDate}) = ${safeYear}`,
  ];
  if (safeMonth && safeMonth >= 1 && safeMonth <= 12) {
    whereClauses.push(
      (0, drizzle_orm_1.sql)`MONTH(${effectiveServiceDate}) = ${safeMonth}`,
    );
  }
  const normalizedDateFrom = String(filters?.dateFrom ?? "").trim();
  if (normalizedDateFrom) {
    whereClauses.push(
      (0, drizzle_orm_1.sql)`${effectiveServiceDate} >= ${normalizedDateFrom}`,
    );
  }
  const normalizedDateTo = String(filters?.dateTo ?? "").trim();
  if (normalizedDateTo) {
    whereClauses.push(
      (0, drizzle_orm_1.sql)`${effectiveServiceDate} <= ${normalizedDateTo}`,
    );
  }
  ...
  if (normalizedServiceType) {
    const matchExpr =
      normalizedServiceType === "lasik"
        ? (0,
          drizzle_orm_1.sql)`LOWER(TRIM(${schema_1.patientServiceEntries.serviceCode})) IN ('1501', '1502')`
  ...
  if (normalizedServiceType) {
    ...
    const rows = await db
      .select({
        ...
        center: (0,
        drizzle_orm_1.sql)`COUNT(DISTINCT CASE WHEN ${schema_1.patients.locationType} = 'center' THEN ${schema_1.patients.id} END)`,
        external: (0,
        drizzle_orm_1.sql)`COUNT(DISTINCT CASE WHEN ${schema_1.patients.locationType} = 'external' THEN ${schema_1.patients.id} END)`,
        lasik: (0,
        drizzle_orm_1.sql)`COUNT(DISTINCT CASE WHEN ${lasikExpr} THEN ${schema_1.patients.id} END)`,
      })
  ...
  const effectivePatientDate = (0,
  drizzle_orm_1.sql)`COALESCE(${schema_1.patients.lastVisit}, DATE(${schema_1.patients.createdAt}))`;
  ...
  if (hasDateFilter) {
    patientWhereClauses.push(
      (0, drizzle_orm_1.sql)`YEAR(${effectivePatientDate}) = ${safeYear}`,
    );
    if (safeMonth && safeMonth >= 1 && safeMonth <= 12) {
      patientWhereClauses.push(
        (0, drizzle_orm_1.sql)`MONTH(${effectivePatientDate}) = ${safeMonth}`,
      );
    }
    if (normalizedDateFrom) {
      patientWhereClauses.push(
        (0,
        drizzle_orm_1.sql)`${effectivePatientDate} >= ${normalizedDateFrom}`,
      );
    }
    if (normalizedDateTo) {
      patientWhereClauses.push(
        (0, drizzle_orm_1.sql)`${effectivePatientDate} <= ${normalizedDateTo}`,
      );
    }
  }
  ...
  const patientWhereClause =
    patientWhereClauses.length > 0
      ? (0, drizzle_orm_1.and)(...patientWhereClauses)
      : undefined;
  ...
  const lasikRows = await db
    .select({
      lasik: (0,
      drizzle_orm_1.sql)`COUNT(DISTINCT ${schema_1.patientServiceEntries.patientId})`,
    })
    .from(schema_1.patientServiceEntries)
    .innerJoin(
      schema_1.patients,
    ...
    .where(
      (0, drizzle_orm_1.and)(
        ...
        drizzle_orm_1.sql)`LOWER(TRIM(${schema_1.patientServiceEntries.serviceCode})) IN ('1501', '1502')`,
        (0,
        drizzle_orm_1.sql)`YEAR(COALESCE(${schema_1.patientServiceEntries.serviceDate}, DATE(${schema_1.patientServiceEntries.updatedAt}))) = ${safeYear}`,
  ...
  for (const sheet of sheets) {
    ...
    try {
      ...
      const patient = await db
        .select()
        .from(schema_1.patients)
        .where((0, drizzle_orm_1.eq)(schema_1.patients.id, sheet.patientId));
      if (!patient.length) {
        skipped++;
        continue;
      }
      ...
      const patientName =
        String(content.patient?.name ?? "").trim() ||
...
async function attachTreatingDoctor(patientRows) {
  ...
  const stateRows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.patientPageStates.page, "examination"),
        (0, drizzle_orm_1.inArray)(
          schema_1.patientPageStates.patientId,
          patientIds,
        ),
      ),
    )
  ...
  for (const row of stateRows) {
    ...
    const doctorNames = Array.isArray(payload.doctorNames)
      ? payload.doctorNames
          .map((v) => normalizeDoctorDisplay(v))
          .filter(Boolean)
      : [];
    ...
    const serviceCode = String(
      payload.serviceCode ?? payload.srvCode ?? payload.srv_cd ?? "",
    ).trim();
  ...
  if (doctorCodes.length > 0) {
    try {
      ...
      const drRows = await db
        .select({
          code: schema_1.doctorsLookup.code,
          name: schema_1.doctorsLookup.name,
        })
        .from(schema_1.doctorsLookup)
        .where(
          (0, drizzle_orm_1.inArray)(schema_1.doctorsLookup.code, uniqueCodes),
        );
  ...
  const reportRows = await db
    ...
    .leftJoin(
      schema_1.users,
      (0, drizzle_orm_1.eq)(schema_1.doctorReports.doctorId, schema_1.users.id),
    )
    .where(
      (0, drizzle_orm_1.inArray)(schema_1.doctorReports.patientId, patientIds),
    )
  ...
  if (!Number.isFinite(insertId) || insertId <= 0) {
    const [latest] = await db
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(
            schema_1.appointments.patientId,
            Number(appointmentData?.patientId ?? 0),
          ),
          (0, drizzle_orm_1.eq)(
            schema_1.appointments.appointmentDate,
            appointmentData?.appointmentDate,
          ),
          (0, drizzle_orm_1.eq)(
            schema_1.appointments.appointmentType,
            String(appointmentData?.appointmentType ?? ""),
          ),
          (0, drizzle_orm_1.eq)(
            schema_1.appointments.branch,
            String(appointmentData?.branch ?? ""),
          ),
        ),
      )
...
async function getAllAppointments(branch) {
  ...
  if (branch) {
    ...
    result = await baseQuery.orderBy(
      (0, drizzle_orm_1.desc)(schema_1.appointments.appointmentDate),
    );
  }
...
async function getAppointmentsByDate(date, branch) {
  ...
  if (branch) {
    return await db
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(schema_1.appointments.branch, branch),
        ),
      );
  }
...
async function createVisit(visitData) {
  ...
  const createdVisits = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.visits.patientId, visitData.patientId),
        (0, drizzle_orm_1.eq)(schema_1.visits.visitDate, visitData.visitDate),
      ),
    )
...
async function getAllVisits() {
  ...
  const result = await db
    ...
    .from(schema_1.visits)
    .innerJoin(
      schema_1.examinations,
      (0, drizzle_orm_1.eq)(schema_1.visits.id, schema_1.examinations.visitId),
    )
    .leftJoin(
      schema_1.patients,
      (0, drizzle_orm_1.eq)(schema_1.visits.patientId, schema_1.patients.id),
    )
    .orderBy((0, drizzle_orm_1.desc)(schema_1.visits.visitDate));
  return result;
}
async function getFollowupVisitsByPatient(patientId) {
  ...
  return await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.visits.patientId, patientId),
        (0, drizzle_orm_1.eq)(schema_1.visits.visitType, "followup"),
      ),
    )
    .orderBy((0, drizzle_orm_1.desc)(schema_1.visits.visitDate));
}
...
async function getFollowupSheetsByPatient(patientId, sheetType) {
  ...
  const condition = sheetType
    ? (0, drizzle_orm_1.and)(
        ...
        (0, drizzle_orm_1.eq)(schema_1.followupSheets.sheetType, sheetType),
      )
    : (0, drizzle_orm_1.eq)(schema_1.followupSheets.patientId, patientId);
  return db
    .select()
    .from(schema_1.followupSheets)
    .where(condition)
...
async function getFollowupItemsBySheet(followupSheetId) {
  ...
  return await db
    .select()
    .from(schema_1.followupItems)
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.followupItems.followupSheetId,
        followupSheetId,
      ),
    )
...
async function updateFollowupItem(itemId, updates) {
  ...
  if (!db) throw new Error("Database not available");
  await db
    .update(schema_1.followupItems)
    .set(updates)
    .where((0, drizzle_orm_1.eq)(schema_1.followupItems.id, itemId));
}
async function deleteFollowupSheet(sheetId) {
  ...
  // Delete items first
  await db
    .delete(schema_1.followupItems)
    .where(
      (0, drizzle_orm_1.eq)(schema_1.followupItems.followupSheetId, sheetId),
    );
  // Then delete sheet
  await db
    .delete(schema_1.followupSheets)
    .where((0, drizzle_orm_1.eq)(schema_1.followupSheets.id, sheetId));
}
  ...
  if (!Number.isFinite(insertId) || insertId <= 0) {
    const [latest] = await db
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(
            schema_1.examinations.patientId,
            Number(examinationData?.patientId ?? 0),
          ),
          (0, drizzle_orm_1.eq)(
            schema_1.examinations.visitId,
            Number(examinationData?.visitId ?? 0),
          ),
        ),
      )
...
async function getExaminationById(id) {
  ...
  const [row] = await db
    .select()
    .from(schema_1.examinations)
    .where((0, drizzle_orm_1.eq)(schema_1.examinations.id, id))
    .limit(1);
  return row ?? null;
}
...
async function getExaminationsByPatient(patientId) {
  ...
  return await db
    .select()
    .from(schema_1.examinations)
    .where((0, drizzle_orm_1.eq)(schema_1.examinations.patientId, patientId))
...
async function getAllExaminations() {
  ...
  const result = await db
    .select({
      ...
      patientName: (0,
      drizzle_orm_1.sql)`JSON_UNQUOTE(JSON_EXTRACT(${schema_1.sheetEntries.content}, '$.patient.name'))`,
    })
    .from(schema_1.examinations)
    .leftJoin(
      schema_1.sheetEntries,
      (0, drizzle_orm_1.eq)(
        schema_1.examinations.patientId,
        schema_1.sheetEntries.patientId,
      ),
    )
    .orderBy((0, drizzle_orm_1.desc)(schema_1.examinations.createdAt));
  return result;
...
async function getAutorefractometryByPatient(patientId) {
  ...
  return await db
    ...
    .leftJoin(
      schema_1.examinations,
      (0, drizzle_orm_1.eq)(
        schema_1.autorefractometryData.examinationId,
        schema_1.examinations.id,
      ),
    )
    .leftJoin(
      schema_1.visits,
      (0, drizzle_orm_1.eq)(schema_1.examinations.visitId, schema_1.visits.id),
    )
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.autorefractometryData.patientId,
        patientId,
      ),
    )
    .orderBy(
      (0, drizzle_orm_1.desc)(
        schema_1.visits.visitDate ?? schema_1.autorefractometryData.createdAt,
      ),
    );
}
async function getGlassesRecordsByPatient(patientId) {
  ...
  return await db
    ...
    .leftJoin(
      schema_1.examinations,
      (0, drizzle_orm_1.eq)(
        schema_1.glassesRecords.examinationId,
        schema_1.examinations.id,
      ),
    )
    .leftJoin(
    ...
    .orderBy(
      (0, drizzle_orm_1.desc)(
        schema_1.visits.visitDate ?? schema_1.glassesRecords.createdAt,
      ),
    );
}
  ...
  try {
    return await db
      ...
      .leftJoin(
        schema_1.examinations,
        (0, drizzle_orm_1.eq)(
          schema_1.afterRefractionData.examinationId,
          schema_1.examinations.id,
        ),
      )
      .leftJoin(
      ...
      .where(
        (0, drizzle_orm_1.eq)(
          schema_1.afterRefractionData.patientId,
          patientId,
        ),
      )
      .orderBy(
        (0, drizzle_orm_1.desc)(
          schema_1.visits.visitDate ?? schema_1.afterRefractionData.createdAt,
        ),
      );
  } catch (error) {
...
async function saveAutorefractometryData(dataInput) {
  ...
  if (existing.length > 0) {
    await db
      .update(schema_1.autorefractometryData)
      .set(dbRecord)
      .where(
        (0, drizzle_orm_1.eq)(
          schema_1.autorefractometryData.examinationId,
...
async function saveGlassesRecord(dataInput) {
  ...
  if (existing.length > 0) {
    await db
      .update(schema_1.glassesRecords)
      .set(dbRecord)
      .where(
        (0, drizzle_orm_1.eq)(
          schema_1.glassesRecords.examinationId,
...
async function getPentacamResultsByPatient(patientId, limit = 100) {
  ...
  return await db
    ...
    .leftJoin(
      schema_1.visits,
      (0, drizzle_orm_1.eq)(
        schema_1.pentacamResults.visitId,
        schema_1.visits.id,
      ),
    )
    ...
    .orderBy(
      (0, drizzle_orm_1.desc)(
        schema_1.visits.visitDate ?? schema_1.pentacamResults.createdAt,
      ),
    )
    .limit(safeLimit);
}
...
async function getPentacamResultsForDashboard(filters) {
  ...
  if (
    ...
    clauses.push(
      (0, drizzle_orm_1.eq)(
        schema_1.pentacamResults.id,
        Number(filters.resultId),
      ),
    );
  }
  if (
  ...
  if (
    ...
    clauses.push(
      (0, drizzle_orm_1.eq)(
        schema_1.pentacamResults.patientId,
  ...
  if (fromD || toD) {
    ...
    clauses.push(
      (0,
      drizzle_orm_1.sql)`DATE(COALESCE(${schema_1.visits.visitDate}, ${schema_1.pentacamResults.createdAt})) >= ${fromBound}`,
    );
    clauses.push(
      (0,
      drizzle_orm_1.sql)`DATE(COALESCE(${schema_1.visits.visitDate}, ${schema_1.pentacamResults.createdAt})) <= ${toBound}`,
    );
  }
  ...
  if (normalizedSearch) {
    ...
    clauses.push(
      (0, drizzle_orm_1.or)(
        (0, drizzle_orm_1.like)(schema_1.patients.fullName, term),
        (0, drizzle_orm_1.like)(schema_1.patients.fullName, legacyTerm),
        (0, drizzle_orm_1.like)(schema_1.doctorsLookup.name, term),
        (0, drizzle_orm_1.like)(schema_1.doctorsLookup.name, legacyTerm),
      ),
    );
  }
  const whereExpr =
    clauses.length > 0 ? (0, drizzle_orm_1.and)(...clauses) : undefined;
  const rows = await db
    .select({
      ...(0, drizzle_orm_1.getTableColumns)(schema_1.pentacamResults),
      visitDate: schema_1.visits.visitDate,
    ...
    .leftJoin(
      schema_1.doctorsLookup,
      (0, drizzle_orm_1.eq)(
        schema_1.patients.doctorCode,
    ...
    .orderBy(
      (0, drizzle_orm_1.desc)(
        (0,
        drizzle_orm_1.sql)`COALESCE(${schema_1.visits.visitDate}, ${schema_1.pentacamResults.createdAt})`,
      ),
    )
    .limit(safeLimit)
    .offset(safeOffset);
...
async function getPentacamDashboardDayStats() {
  ...
  const rows = await db
    .select({
      todayCount: (0,
      drizzle_orm_1.sql)`COALESCE(SUM(CASE WHEN DATE(COALESCE(${schema_1.visits.visitDate}, ${schema_1.pentacamResults.createdAt})) = CURDATE() THEN 1 ELSE 0 END), 0)`,
      yesterdayCount: (0,
      drizzle_orm_1.sql)`COALESCE(SUM(CASE WHEN DATE(COALESCE(${schema_1.visits.visitDate}, ${schema_1.pentacamResults.createdAt})) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) THEN 1 ELSE 0 END), 0)`,
    })
    .from(schema_1.pentacamResults)
    .leftJoin(
      schema_1.visits,
...
async function deletePentacamResultsByIds(ids) {
  ...
  await db
    .delete(schema_1.pentacamResults)
    .where((0, drizzle_orm_1.inArray)(schema_1.pentacamResults.id, normalized));
...
async function updateDoctorReport(reportId, updates) {
  ...
  await db
    .update(schema_1.doctorReports)
    .set(updates)
    .where((0, drizzle_orm_1.eq)(schema_1.doctorReports.id, reportId));
}
...
async function getMedicalReportsOverviewRows(limit = 250) {
  ...
  const rows = await dbConn
    ...
    .leftJoin(
      schema_1.patients,
      (0, drizzle_orm_1.eq)(
        schema_1.doctorReports.patientId,
        schema_1.patients.id,
      ),
    )
    .leftJoin(
...
async function getDoctorReportsByPatient(patientId) {
  ...
  return await db
    .select()
    .from(schema_1.doctorReports)
    .where((0, drizzle_orm_1.eq)(schema_1.doctorReports.patientId, patientId))
    .orderBy((0, drizzle_orm_1.desc)(schema_1.doctorReports.createdAt));
}
...
async function createPrescription(prescriptionData) {
  ...
  if (medicationName) {
    const existing = await db
      .select()
      .from(schema_1.medications)
      .where((0, drizzle_orm_1.eq)(schema_1.medications.name, medicationName))
      .limit(1);
    let medicationId;
    if (existing.length > 0) {
...
async function getPrescriptionsWithItemsByVisit(visitId) {
  ...
  const rows = await db
    ...
    .from(schema_1.prescriptions)
    .leftJoin(
      schema_1.prescriptionItems,
      (0, drizzle_orm_1.eq)(
        schema_1.prescriptions.id,
        schema_1.prescriptionItems.prescriptionId,
      ),
    )
    .leftJoin(
      schema_1.medications,
      (0, drizzle_orm_1.eq)(
        schema_1.prescriptionItems.medicationId,
        schema_1.medications.id,
      ),
    )
    .where((0, drizzle_orm_1.eq)(schema_1.prescriptions.visitId, visitId))
    .orderBy((0, drizzle_orm_1.desc)(schema_1.prescriptions.prescriptionDate));
...
async function getPrescriptionsOverviewRows(limit = 250) {
  ...
  const base = await dbConn
    ...
    .leftJoin(
      schema_1.patients,
      (0, drizzle_orm_1.eq)(
        schema_1.prescriptions.patientId,
        schema_1.patients.id,
      ),
    )
    .leftJoin(
      schema_1.users,
      (0, drizzle_orm_1.eq)(schema_1.prescriptions.doctorId, schema_1.users.id),
    )
    .orderBy((0, drizzle_orm_1.desc)(schema_1.prescriptions.prescriptionDate))
    .limit(cap);
...
async function createPrescriptionWithItems(data) {
  ...
  if (!prescriptionId) {
    const lastIdResult = await db.execute(
      (0, drizzle_orm_1.sql)`select last_insert_id() as id`,
    );
  ...
  for (const item of validItems) {
    ...
    if (!medicationId) {
      ...
      const existing = await db
        .select()
        .from(schema_1.medications)
        .where((0, drizzle_orm_1.eq)(schema_1.medications.name, name))
        .limit(1);
      if (existing.length > 0) {
        medicationId = existing[0].id;
      } else {
        const inserted = await db
          .insert(schema_1.medications)
...
async function deletePrescription(prescriptionId) {
  ...
  await db
    .delete(schema_1.prescriptions)
    .where((0, drizzle_orm_1.eq)(schema_1.prescriptions.id, prescriptionId));
}
// ============ SURGERY OPERATIONS ============
...
async function getSurgeriesByPatient(patientId) {
  ...
  return await db
    .select()
    .from(schema_1.surgeries)
    .where((0, drizzle_orm_1.eq)(schema_1.surgeries.patientId, patientId))
    .orderBy((0, drizzle_orm_1.desc)(schema_1.surgeries.surgeryDate));
}
async function deleteSurgery(surgeryId) {
  ...
  await db
    .delete(schema_1.surgeries)
    .where((0, drizzle_orm_1.eq)(schema_1.surgeries.id, surgeryId));
}
async function updateSurgery(surgeryId, updates) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .update(schema_1.surgeries)
    .set(updates)
    .where((0, drizzle_orm_1.eq)(schema_1.surgeries.id, surgeryId));
}
...
async function getPostOpFollowupsBySurgery(surgeryId) {
  ...
  return await db
    .select()
    .from(schema_1.postOpFollowups)
    .where((0, drizzle_orm_1.eq)(schema_1.postOpFollowups.surgeryId, surgeryId))
    .orderBy((0, drizzle_orm_1.desc)(schema_1.postOpFollowups.followupDate));
}
async function getPostOpFollowupsByPatient(patientId) {
  ...
  return await db
    .select()
    .from(schema_1.postOpFollowups)
    .where((0, drizzle_orm_1.eq)(schema_1.postOpFollowups.patientId, patientId))
    .orderBy((0, drizzle_orm_1.desc)(schema_1.postOpFollowups.followupDate));
}
...
async function createMedicalHistory(historyData) {
  ...
  if (!db) throw new Error("Database not available");
  const result = await db
    .insert(schema_1.medicalHistoryChecklist)
    .values(historyData);
  return result;
}
async function getMedicalHistoryByPatient(patientId) {
  ...
  return await db
    .select()
    .from(schema_1.medicalHistoryChecklist)
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.medicalHistoryChecklist.patientId,
        patientId,
...
async function updateMedication(medicationId, updates) {
  ...
  if (!db) throw new Error("Database not available");
  await db
    .update(schema_1.medications)
    .set(updates)
    .where((0, drizzle_orm_1.eq)(schema_1.medications.id, medicationId));
}
async function deleteMedication(medicationId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .delete(schema_1.medications)
    .where((0, drizzle_orm_1.eq)(schema_1.medications.id, medicationId));
}
// ============ TEST OPERATIONS ============
...
async function updateTest(testId, updates) {
  ...
  if (!db) throw new Error("Database not available");
  await db
    .update(schema_1.tests)
    .set(updates)
    .where((0, drizzle_orm_1.eq)(schema_1.tests.id, testId));
}
async function deleteTest(testId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .delete(schema_1.tests)
    .where((0, drizzle_orm_1.eq)(schema_1.tests.id, testId));
}
async function getTestFavoritesByUser(userId) {
  ...
  return await db
    ...
    .where((0, drizzle_orm_1.eq)(schema_1.testFavorites.userId, userId));
}
async function toggleTestFavorite(userId, testId) {
  ...
  const existing = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.testFavorites.userId, userId),
        (0, drizzle_orm_1.eq)(schema_1.testFavorites.testId, testId),
      ),
    )
    .limit(1);
  if (existing.length > 0) {
    ...
    requestData.map(async (req) => {
      const items = await db
        ...
        .innerJoin(
          schema_1.tests,
          (0, drizzle_orm_1.eq)(
            schema_1.testRequestItems.testId,
            schema_1.tests.id,
          ),
        )
        .where(
...
async function getSystemSetting(key) {
  ...
  for (let i = 0; i < partCount; i += 1) {
    ...
    const partRows = await db
      .select()
      .from(schema_1.systemSettings)
      .where((0, drizzle_orm_1.eq)(schema_1.systemSettings.key, partKey))
      .limit(1);
...
async function updateSystemSettings(key, value) {
  ...
  const cleanupChunks = async () => {
    const chunkRows = await db
      ...
      .from(schema_1.systemSettings)
      .where(
        (0, drizzle_orm_1.like)(schema_1.systemSettings.key, `${key}__chunk_%`),
      );
    if (chunkRows.length > 0) {
      await db.delete(schema_1.systemSettings).where(
        (0, drizzle_orm_1.inArray)(
          schema_1.systemSettings.key,
  ...
  if (stale.length > 0) {
    await db
      ...
      .where((0, drizzle_orm_1.inArray)(schema_1.systemSettings.key, stale));
  }
}
const TEAM_PERMISSION_ROLES = [
  "admin",
  "manager",
  "accountant",
  "doctor",
...
async function getSheetEntry(patientId, sheetType) {
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.sheetEntries.patientId, patientId),
        (0, drizzle_orm_1.eq)(schema_1.sheetEntries.sheetType, sheetType),
      ),
    )
  ...
  const existing = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        ...
        (0, drizzle_orm_1.eq)(
          schema_1.sheetEntries.sheetType,
          params.sheetType,
        ),
      ),
    )
...
async function getOperationList(doctorTab, listDate, operationType) {
  ...
  const lists = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.operationLists.doctorTab, doctorTab),
        (0, drizzle_orm_1.eq)(schema_1.operationLists.listDate, dateValue),
        operationTypeKey === null
          ? (0, drizzle_orm_1.isNull)(schema_1.operationLists.operationType)
          : (0, drizzle_orm_1.eq)(
              schema_1.operationLists.operationType,
              operationTypeKey,
            ),
      ),
    )
    .limit(1);
  if (lists.length === 0) return { id: null, items: [] };
  const rawItems = await db
    .select()
    .from(schema_1.operationListItems)
    .where(
      (0, drizzle_orm_1.eq)(schema_1.operationListItems.listId, lists[0].id),
    )
...
async function getOperationListById(listId) {
  ...
  const rawItemsById = await db
    .select()
    .from(schema_1.operationListItems)
    .where((0, drizzle_orm_1.eq)(schema_1.operationListItems.listId, listId))
    .orderBy(schema_1.operationListItems.id);
...
async function saveOperationList(data) {
  ...
  if (!listId) {
    const existing = await db
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(
            schema_1.operationLists.doctorTab,
            data.doctorTab,
          ),
          (0, drizzle_orm_1.eq)(schema_1.operationLists.listDate, dateValue),
          operationTypeKey === null
  ...
  if (receiptNumbers.length > 0) {
    const conflicts = await db
      ...
      .where(
        (0, drizzle_orm_1.inArray)(
          schema_1.operationListItems.number,
          receiptNumbers,
        ),
      );
  ...
  if (patientCodes.length > 0) {
    const codeConflicts = await db
      ...
      .where(
        (0, drizzle_orm_1.inArray)(
          schema_1.operationListItems.code,
          patientCodes,
        ),
      );
  ...
  if (!listId) {
    ...
    await db
      .update(schema_1.operationLists)
      .set({
        doctorTab: data.doctorTab,
        listDate: dateValue,
        operationType: operationTypeKey,
      ...
      .where((0, drizzle_orm_1.eq)(schema_1.operationLists.id, listId));
    await db
      .delete(schema_1.operationListItems)
      .where((0, drizzle_orm_1.eq)(schema_1.operationListItems.listId, listId));
  }
  if (listId) {
    await db.insert(schema_1.operationListItems).values(
      data.items.map((item) => ({
        listId,
...
async function deleteOperationList(doctorTab, listDate) {
  ...
  await db
    .delete(schema_1.operationListItems)
    .where(
      (0, drizzle_orm_1.eq)(schema_1.operationListItems.listId, existing[0].id),
    );
  await db
    .delete(schema_1.operationLists)
    .where((0, drizzle_orm_1.eq)(schema_1.operationLists.id, existing[0].id));
}
...
async function getOperationListsHistory() {
  ...
  return await db
    ...
    .from(schema_1.operationLists)
    .orderBy(
      (0, drizzle_orm_1.desc)(schema_1.operationLists.listDate),
      (0, drizzle_orm_1.desc)(schema_1.operationLists.updatedAt),
      (0, drizzle_orm_1.desc)(schema_1.operationLists.id),
    );
}
...
async function getAutoOperationListsByDate(dateString) {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        ...
        const category = String(entry?.category ?? "")
          .trim()
          .toLowerCase();
        const defaultSheet = String(entry?.defaultSheet ?? "")
          .trim()
          .toLowerCase();
        const isSurgery =
          serviceType === "surgery" ||
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.patientServiceEntries.source, "mssql"),
        (0, drizzle_orm_1.sql)`(
    ...
    .orderBy(
      (0, drizzle_orm_1.desc)(schema_1.patientServiceEntries.updatedAt),
      (0, drizzle_orm_1.desc)(schema_1.patientServiceEntries.id),
    );
  const filtered = rows.filter((row) =>
    surgeryCodes.has(normalizeCode(row.serviceCode)),
  );
...
async function upsertPushDeviceRegistration(input) {
  ...
  if (existingByToken.length > 0) {
    await db
      ...
      .where(
        (0, drizzle_orm_1.eq)(
          schema_1.pushDeviceRegistrations.id,
          existingByToken[0].id,
        ),
      );
  ...
  if (payload.deviceId) {
    const existingByDevice = await db
      ...
      .where(
        (0, drizzle_orm_1.and)(
          (0, drizzle_orm_1.eq)(
            schema_1.pushDeviceRegistrations.userId,
            userId,
          ),
          (0, drizzle_orm_1.eq)(
            schema_1.pushDeviceRegistrations.deviceId,
            payload.deviceId,
          ),
        ),
      )
  ...
  const result = await db
    .insert(schema_1.pushDeviceRegistrations)
    .values(payload);
...
async function disablePushDeviceToken(token) {
  ...
  await db
    ...
    .where(
      (0, drizzle_orm_1.eq)(schema_1.pushDeviceRegistrations.token, normalized),
    );
}
async function deletePushDeviceToken(token) {
  ...
  if (!normalized) return;
  await db
    .delete(schema_1.pushDeviceRegistrations)
    .where(
      (0, drizzle_orm_1.eq)(schema_1.pushDeviceRegistrations.token, normalized),
    );
}
async function getActivePushDeviceRegistrations() {
  ...
  return await db
    ...
    .where(
      (0,
      drizzle_orm_1.sql)`${schema_1.pushDeviceRegistrations.disabledAt} IS NULL`,
    )
    .orderBy(
      (0, drizzle_orm_1.desc)(schema_1.pushDeviceRegistrations.lastSeenAt),
      (0, drizzle_orm_1.desc)(schema_1.pushDeviceRegistrations.id),
    );
}
async function getActivePushDeviceRegistrationsByUser(userId) {
  ...
  return await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.pushDeviceRegistrations.userId, userId),
        (0,
        drizzle_orm_1.sql)`${schema_1.pushDeviceRegistrations.disabledAt} IS NULL`,
      ),
    )
...
async function getUserPageState(userId, page) {
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.userPageStates.userId, userId),
        (0, drizzle_orm_1.eq)(schema_1.userPageStates.page, page),
      ),
    )
    .limit(1);
  return rows[0] ?? null;
...
async function upsertUserPageState(userId, page, data) {
  ...
  if (!existing.length) {
    await db
      .insert(schema_1.userPageStates)
      .values({ userId, page, data: data });
    return;
  }
  await db
    .update(schema_1.userPageStates)
    .set({ data: data })
    .where((0, drizzle_orm_1.eq)(schema_1.userPageStates.id, existing[0].id));
}
...
async function getPatientPageState(patientId, page) {
  ...
  const rows = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.eq)(schema_1.patientPageStates.patientId, patientId),
        (0, drizzle_orm_1.eq)(schema_1.patientPageStates.page, page),
      ),
    )
    .orderBy(
      (0, drizzle_orm_1.desc)(schema_1.patientPageStates.updatedAt),
      (0, drizzle_orm_1.desc)(schema_1.patientPageStates.id),
    )
    .limit(1);
  return rows[0] ?? null;
}
async function upsertPatientPageState(patientId, page, data) {
  ...
  if (!existingRows.length) {
    await db
      .insert(schema_1.patientPageStates)
      .values({ patientId, page, data: data });
    return;
  }
...
async function upsertPatientServiceEntry(input) {
  ...
  const existing = await db
    ...
    .where(
      (0, drizzle_orm_1.eq)(
        schema_1.patientServiceEntries.sourceRef,
        sourceRef,
      ),
    )
    .limit(1);
  ...
  await db
    ...
    .where(
      (0, drizzle_orm_1.eq)(schema_1.patientServiceEntries.id, existing[0].id),
    );
}
async function getPatientServiceEntriesByPatients(patientIds) {
  ...
  return await db
    ...
    .from(schema_1.patientServiceEntries)
    .where(
      (0, drizzle_orm_1.inArray)(schema_1.patientServiceEntries.patientId, ids),
    )
...
async function getAllDiseases() {
  ...
  return await db
    .select()
    .from(schema_1.diseases)
    .orderBy((0, drizzle_orm_1.desc)(schema_1.diseases.id));
}
...
async function updateDisease(diseaseId, name, branch, abbrev) {
  ...
  if (!db) throw new Error("Database not available");
  await db
    .update(schema_1.diseases)
    .set({ name, branch: branch || null, abbrev: abbrev || null })
    .where((0, drizzle_orm_1.eq)(schema_1.diseases.id, diseaseId));
}
async function deleteDisease(diseaseId) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db
    .delete(schema_1.diseases)
    .where((0, drizzle_orm_1.eq)(schema_1.diseases.id, diseaseId));
}
...
async function getTodayVisitsByQueueStatus(dateIso, queueStatus) {
  ...
  if (!db) throw new Error("Database not available");
  const whereClauses = [
    (0, drizzle_orm_1.sql)`DATE(${schema_1.visits.visitDate}) = ${dateIso}`,
  ];
  if (queueStatus) {
    whereClauses.push(
      (0, drizzle_orm_1.eq)(schema_1.visits.queueStatus, queueStatus),
    );
  }
  const rows = await db
    .select({
      ...
      checkedInTime: (0,
      drizzle_orm_1.sql)`DATE_FORMAT(${schema_1.visits.checkedInAt}, '%H:%i')`,
    ...
    .leftJoin(
      schema_1.doctorsLookup,
      (0,
      drizzle_orm_1.sql)`${schema_1.doctorsLookup.id} = ${schema_1.patients.doctorId}`,
    )
...
async function autoAdvanceQueuePatients(dateIso) {
  ...
  const dayMatch = (0,
  drizzle_orm_1.sql)`DATE(${schema_1.visits.visitDate}) = ${dateIso}`;
    ...
    const rows = await conn
      ...
      .where(
        (0, drizzle_orm_1.and)(
          dayMatch,
          nonExternalExpr,
          (0, drizzle_orm_1.eq)(schema_1.visits.queueStatus, status),
        ),
      )
      .orderBy(schema_1.visits.id)
      .limit(1);
    ...
    const rows = await conn
      ...
      .where(
        (0, drizzle_orm_1.and)(
          dayMatch,
          nonExternalExpr,
          (0, drizzle_orm_1.eq)(schema_1.visits.queueStatus, "checkedIn"),
        ),
      )
      .orderBy(schema_1.visits.id)
      .limit(limit);
  ...
  if (clinicId == null && nextHead != null) {
    await conn
      ...
      .set({
        queueStatus: "clinic",
        movedToClinicAt: (0, drizzle_orm_1.sql)`CURRENT_TIMESTAMP`,
      })
  ...
  if (nextCandidate != null) {
    await conn
      ...
      .set({
        queueStatus: "next",
        movedToNextAt: (0, drizzle_orm_1.sql)`CURRENT_TIMESTAMP`,
      })
...
async function getVisitDateIsoById(visitId) {
  ...
  const rows = await db
    .select({
      visitDateIso: (0,
      drizzle_orm_1.sql)`DATE_FORMAT(${schema_1.visits.visitDate}, '%Y-%m-%d')`,
    })
    .from(schema_1.visits)
    .where((0, drizzle_orm_1.eq)(schema_1.visits.id, visitId))
    .limit(1);
  ...
  const nextVisits = await db
    ...
    .where(
      (0, drizzle_orm_1.and)(
        (0, drizzle_orm_1.sql)`DATE(${schema_1.visits.visitDate}) = ${dateIso}`,
        nonExternalExpr,
        (0, drizzle_orm_1.eq)(schema_1.visits.queueStatus, "next"),
      ),
    )
    .orderBy(schema_1.visits.id)
    .limit(1);
