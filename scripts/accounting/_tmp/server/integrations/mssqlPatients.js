...
exports.syncSinglePatientFromMssql = syncSinglePatientFromMssql;
exports.insertPatientToMssql = insertPatientToMssql;
exports.upsertPatientToMssql = upsertPatientToMssql;
exports.ensurePatientServiceInMssql = ensurePatientServiceInMssql;
exports.deletePatientFromMssqlByCode = deletePatientFromMssqlByCode;
exports.backfillPapatSrvNamesInMssql = backfillPapatSrvNamesInMssql;
exports.loadDoctorMapFromCsv = loadDoctorMapFromCsv;
exports.getMssqlSyncStatus = getMssqlSyncStatus;
exports.syncPatientsFromMssql = syncPatientsFromMssql;
const db = __importStar(require("../db"));
const appNotifications_1 = require("../_core/appNotifications");
...
let doctorCsvCache = null;
let serviceCsvCache = null;
let doctorServiceSheetMatchCache = null;
let pentacamServiceCodesCache = null;
...
function normalizeGender(input) {
    ...
    if (!v)
        return undefined;
    if (["m", "male", "man", "ذكر"].includes(v))
        return "male";
    if (["f", "female", "woman", "انثى", "أنثى"].includes(v))
        return "female";
    return undefined;
}
...
function normalizeIsoDateTime(input) {
    if (!input)
        return undefined;
    if (input instanceof Date && !Number.isNaN(input.valueOf()))
...
function normalizeLocationTypeFromIdNo(input) {
    const raw = String(input ?? "").trim();
    if (!raw)
        return undefined;
    if (raw === "1")
...
function normalizeBranch(input) {
    ...
    if (!v)
        return undefined;
    if (["surgery", "operation", "عمليات"].includes(v))
        return "surgery";
...
function normalizeServiceType(input) {
    ...
    if (!v)
        return undefined;
    if (["consultant", "استشاري"].includes(v))
        return "consultant";
    if (["specialist", "اخصائي", "أخصائي"].includes(v))
        return "specialist";
    if (["lasik", "ليزك", "lasik moria", "lasik metal"].includes(v))
        return "lasik";
    if (["surgery", "operation", "عمليات"].includes(v))
        return "surgery";
    if (["external", "outside", "خارجي"].includes(v))
        return "external";
    return undefined;
}
function normalizeServiceCode(input) {
    const raw = String(input ?? "").trim();
    if (!raw)
        return "";
...
function normalizeLocationType(input) {
    ...
    if (["external", "outside", "خارجي"].includes(v))
        return "external";
    if (["center", "internal", "داخلي", "المركز"].includes(v))
        return "center";
    return undefined;
}
function normalizeSheetType(input) {
    const v = String(input ?? "").trim().toLowerCase();
    if (!v)
        return undefined;
    if (v === "consultant")
    ...
    if (v === "external")
        return "external";
    if (v === "pentacam" || v === "pentacam_center" || v === "radiology_center" || v === "pentacam_c")
        return "lasik";
    if (v === "pentacam_external" ||
...
function normalizeDoctorServiceMatchKey(doctorCode, serviceCode) {
...
const REQUIRED_SYNC_COLUMNS = new Set([
    // rece (PAJRNRCVH)
    "PAT_CD",
    "NAM",
    "NAM1",
...
const REQUIRED_SYNC_ALIASES = new Set([
    "patientCode",
    "fullName",
    "phone",
    "address",
...
const OPERATIONAL_ONLY_KEYS = new Set(["changedAt", "serviceCodesCsv", "SRV_CODES", "srv_codes"]);
function isAutoOrDateLikeFallback(key) {
    const k = String(key ?? "").trim();
    if (!k)
        return true;
    if (["PAT_CD", "patientCode", "SRV_CD", "serviceCode", "SRV_CODES", "serviceCodesCsv"].includes(k))
        return false;
    if (/DATE|_DT$|^DT$|_TIM$|TIME/i.test(k))
        return true;
    if (/^(TR_NO|tr_noNew|VST_NO|REPIT_NO|PRG_SNO|LN_NO|AZTR_NO|AZLN_NO|OPTR_NO|WF_TR_NO|WF_LN_NO|enter_no|CUR_SRV_IDX|T_BTCH_ID|BTCH_ID|TMP_NO|TMP_DR_BTCHID)$/i.test(k)) {
        return true;
    }
    return false;
}
function shouldIncludeColumnForMerge(key) {
    const k = String(key ?? "").trim();
    if (!k)
        return false;
    if (REQUIRED_SYNC_COLUMNS.has(k.toUpperCase()))
        return true;
    if (REQUIRED_SYNC_ALIASES.has(k.toLowerCase()))
        return true;
    if (!STRICT_REQUIRED_SYNC_COLUMNS && !isAutoOrDateLikeFallback(k))
        return true;
    return false;
}
function buildMssqlBackfillObject(row) {
    const out = {};
    for (const [key, value] of Object.entries(row ?? {})) {
        if (!shouldIncludeColumnForMerge(key))
            continue;
        if (OPERATIONAL_ONLY_KEYS.has(key))
            continue;
        if (isBlank(value))
            continue;
        out[key] = value;
...
async function loadMssqlMsNodeSqlV8Module() {
    try {
        ...
        throw new Error(`Windows auth driver failed to load (${reason}). Ensure 'msnodesqlv8' is installed and built: npx -y pnpm@10.29.2 add msnodesqlv8 && npx -y pnpm@10.29.2 rebuild msnodesqlv8`);
    }
}
function escapeSqlString(value) {
    return value.replace(/'/g, "''");
}
function toSqlDateTimeLiteral(input) {
    const raw = String(input ?? "").trim();
    if (!raw)
        return "";
    const dt = new Date(raw);
...
function resolveShiftNumber(now = new Date()) {
...
async function createMssqlPool() {
    ...
    const inferredWindowsMode = /trusted_connection\s*=\s*yes/i.test(connectionStringRaw) ||
...
async function applyPapatSrvDefaults(pool, patientCode, serviceCode, desiredQty, options) {
    ...
    if (qtyCol) {
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET ${qtyCol} = @QTY WHERE ${whereSrv}`, (req) => req.input("QTY", qty));
    }
    if (cols.has("DISC_VL")) {
    ...
    if (basePrice == null && priceCol) {
        ...
        if (hasScopedTrNo)
            rowReq.input("TR_NO", Math.trunc(scopedTrNo));
    ...
    if (cols.has("PRC") && basePrice != null) {
        await run(`UPDATE op2026.dbo.PAPAT_SRV
       SET PRC = CASE
         ...
         ELSE @PRC
       END
       WHERE ${whereSrv}`, (req) => req.input("PRC", basePrice));
    }
    ...
    if (cols.has("PA_VL")) {
        ...
        if (hasScopedTrNo)
            discReq.input("TR_NO", Math.trunc(scopedTrNo));
    ...
    `);
        ...
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET PA_VL = @PA_VL WHERE ${whereSrv}`, (req) => req.input("PA_VL", total));
    }
    ...
    if (patientNameForSrv) {
        if (cols.has("NAM")) {
            await run(`UPDATE op2026.dbo.PAPAT_SRV SET NAM = @NAM WHERE ${whereSrv}`, (req) => req.input("NAM", patientNameForSrv));
        }
        if (cols.has("PAT_NM_AR")) {
            await run(`UPDATE op2026.dbo.PAPAT_SRV SET PAT_NM_AR = @PAT_NM_AR WHERE ${whereSrv}`, (req) => req.input("PAT_NM_AR", patientNameForSrv));
        }
    }
    if (cols.has("PAT_TYP"))
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET PAT_TYP = 2 WHERE ${whereSrv}`);
    if (cols.has("CUR_SRV_BY"))
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET CUR_SRV_BY = NULL WHERE ${whereSrv}`);
    if (cols.has("PRG_BY"))
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET PRG_BY = NULL WHERE ${whereSrv}`);
    if (cols.has("CA_CD"))
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET CA_CD = '00000' WHERE ${whereSrv}`);
    ...
    if (cols.has("DISC")) {
        if (cols.has("DISC_VL")) {
            await run(`UPDATE op2026.dbo.PAPAT_SRV SET DISC = CASE WHEN ISNUMERIC(CONVERT(varchar(50), DISC_VL))=1 THEN CAST(CONVERT(varchar(50), DISC_VL) AS decimal(18,2)) ELSE 0 END WHERE ${whereSrv}`);
        }
        else {
    ...
    if (cols.has("LN_SRC"))
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET LN_SRC = 'PAJRNRCV' WHERE ${whereSrv}`);
    ...
    if (cols.has("DISC_P")) {
        if (cols.has("DISC_VL")) {
            await run(`UPDATE op2026.dbo.PAPAT_SRV SET DISC_P = CASE WHEN ISNUMERIC(CONVERT(varchar(50), DISC_VL))=1 THEN CAST(CONVERT(varchar(50), DISC_VL) AS decimal(18,2)) ELSE 0 END WHERE ${whereSrv}`);
        }
        else {
    ...
    if (cols.has("ENTEREDBY") && String(options?.enteredBy ?? "").trim()) {
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET ENTEREDBY = @ENTEREDBY WHERE ${whereSrv}`, (req) => req.input("ENTEREDBY", String(options?.enteredBy ?? "").trim()));
    }
    if (cols.has("ENTRYDATE") && String(options?.entryDate ?? "").trim()) {
        await run(`UPDATE op2026.dbo.PAPAT_SRV SET ENTRYDATE = @ENTRYDATE WHERE ${whereSrv}`, (req) => req.input("ENTRYDATE", String(options?.entryDate ?? "").trim()));
    }
    // Reports also require PAPAT_IO + PAPATMF consistency for the same visit.
    let ensureVisitDt = null;
    try {
        const vstReq = pool.request();
        vstReq.input("PAT_CD", patientCode);
        vstReq.input("SRV_CD", serviceCode);
        if (hasScopedTrNo)
            vstReq.input("TR_NO", Math.trunc(scopedTrNo));
    ...
    try {
        await ensurePapatMfDefaults(pool, patientCode, ensureVisitDt ?? (String(options?.entryDate ?? "").trim() ? new Date(String(options?.entryDate ?? "").trim()) : new Date()), String(options?.patientNameAr ?? "").trim() || null);
    }
    catch {
...
async function getTableColumns(pool, tableName) {
  ...
  `);
    return new Set((Array.isArray(result?.recordset) ? result.recordset : [])
        .map((r) => String(r?.name ?? "").toUpperCase())
        .filter(Boolean));
}
async function withMssqlServiceInsertLock(pool, patientCode, serviceCode, work) {
  ...
  `);
    const lockResult = Number(Array.isArray(lockRs?.recordset) && lockRs.recordset.length > 0 ? lockRs.recordset[0]?.lockResult : NaN);
...
async function ensurePapatMfDefaults(pool, patientCode, visitDt, patientNameAr) {
  ...
  `);
    ...
    if (!exists) {
        ...
        const insertCols = ["PAT_CD"];
        const insertVals = ["@PAT_CD"];
        ...
        if (cols.has("DRS_CD")) {
            ...
            insReq.input("DRS_CD", Number.isFinite(Number(h?.DRS_CD)) ? Math.trunc(Number(h.DRS_CD)) : null);
        }
        try {
            await insReq.query(`
      ...
      `);
        ...
        `);
                ...
                const insertableCols = colsMeta
                    .filter((c) => Number(c?.is_identity ?? 0) === 0 && Number(c?.is_computed ?? 0) === 0)
                    .map((c) => String(c?.name ?? "").trim())
                    .filter(Boolean);
                if (insertableCols.length > 0) {
                    const q = (n) => `[${n.replace(/]/g, "]]")}]`;
                    const mappedSelect = insertableCols.map((col) => {
                        const u = col.toUpperCase();
                        if (u === "PAT_CD")
                            return `@PAT_CD AS ${q(col)}`;
                        if (u === "PAT_NM_AR")
                        ...
                        if (u === "PAT_TYP")
                            return `COALESCE(t.${q(col)}, 2) AS ${q(col)}`;
                        if (u === "RLTN")
                            return `COALESCE(t.${q(col)}, 1) AS ${q(col)}`;
                        if (u === "PRC_DGR")
                            return `COALESCE(t.${q(col)}, 1) AS ${q(col)}`;
                        if (u === "GNDR")
                            return `COALESCE(h.GNDR, t.${q(col)}) AS ${q(col)}`;
                        if (u === "AGE")
                            return `COALESCE(h.AGE, t.${q(col)}) AS ${q(col)}`;
                        if (u === "IDNO")
...
async function ensurePapatIoDefaults(pool, patientCode, trNo, vstNo, visitDt, doctorCode) {
    if (!Number.isFinite(Number(vstNo)))
        return;
    const cols = await getTableColumns(pool, "op2026.dbo.PAPAT_IO");
    if (!cols.has("PAT_CD") || !cols.has("VST_NO"))
        return;
    const req = pool.request();
    req.input("PAT_CD", patientCode);
...
async function applyPajrnrCvhDefaults(pool, targetTable, patientCode, genderCode, payAmount, doctorCode) {
    ...
    if (cols.has("CA_ACC"))
        await run(`UPDATE ${targetTable} SET CA_ACC = ISNULL(NULLIF(CA_ACC, ''), '00000') WHERE PAT_CD = @PAT_CD`);
    if (cols.has("XSEC_CD"))
    ...
    if (cols.has("DRS_CD")) {
        console.log(`[MSSQL Defaults] DRS_CD column exists. doctorCode=${doctorCode}`);
        if (doctorCode) {
            console.log(`[MSSQL Defaults] Setting DRS_CD=${doctorCode} for ${patientCode}`);
            await run(`UPDATE ${targetTable} SET DRS_CD = @DRS_CD WHERE PAT_CD = @PAT_CD`, (req) => req.input("DRS_CD", doctorCode));
        }
        else {
            console.log(`[MSSQL Defaults] doctorCode is null/empty, skipping DRS_CD update`);
        }
    }
    else {
    ...
    `);
        ...
        if (cols.has("TOTL")) {
            await run(`UPDATE ${targetTable} SET TOTL = @TOTL WHERE PAT_CD = @PAT_CD`, (req) => req.input("TOTL", Number.isFinite(totalSrv) ? totalSrv : 0));
        }
        if (cols.has("DISC")) {
            await run(`UPDATE ${targetTable} SET DISC = @DISC WHERE PAT_CD = @PAT_CD`, (req) => req.input("DISC", Number.isFinite(totalDisc) ? totalDisc : 0));
        }
        if (cols.has("PA_VL")) {
            await run(`UPDATE ${targetTable} SET PA_VL = @PAVL WHERE PAT_CD = @PAT_CD`, (req) => req.input("PAVL", Number.isFinite(totalSrv) ? totalSrv : 0));
        }
    }
}
async function applyPajrnrReportDefaults(pool, targetTable, patientCode, trNo) {
    if (!Number.isFinite(Number(trNo)))
        return;
    const cols = await getTableColumns(pool, targetTable);
...
async function loadDoubleEyeServiceCodes() {
    const now = Date.now();
    if (pentacamServiceCodesCache && now - pentacamServiceCodesCache.at < 60000) {
...
async function getDesiredServiceQty(serviceCodeRaw) {
...
async function syncSinglePatientFromMssql(patientCodeRaw) {
    ...
    `;
        ...
        const alwaysSyncFields = ["phone", "address", "age", "gender", "dateOfBirth", "branch", "lastVisit", "receptionSignature"];
        ...
        if (Object.keys(updates).length > 0) {
            ...
            return { synced: true, message: `Synced ${Object.keys(updates).length} field(s) from MSSQL` };
        }
        return { synced: true, message: "No data to sync from MSSQL" };
    }
    catch (error) {
        const message = String(error?.message ?? error ?? "unknown");
        console.warn(`[mssql-sync] Failed to sync patient ${patientCode}:`, message);
...
async function insertPatientToMssql(input) {
    ...
    const dedupWindowSeconds = Number.isFinite(dedupWindowSecondsRaw) && dedupWindowSecondsRaw > 0
    ...
    try {
        ...
        console.log(`[MSSQL Insert] Fetched ${targetCols.size} columns from ${targetTable}`);
    ...
    `);
        ...
        const withinDedupWindow = Number.isFinite(latestTouchedMs) &&
            Date.now() - latestTouchedMs >= 0 &&
            Date.now() - latestTouchedMs <= dedupWindowSeconds * 1000;
        console.log(`[MSSQL Insert] Latest patient record exists: ${Boolean(latestRow)}, withinDedupWindow: ${withinDedupWindow}`);
        let trNo = Number.NaN;
        let headerVstNo = 1;
        let headerDt = new Date(todayDateOnly);
        if (withinDedupWindow) {
            console.log(`[MSSQL Insert] Patient exists within dedup window, reusing existing record`);
            ...
            console.log(`[MSSQL Insert] New patient record, proceeding with insert...`);
            const req = pool.request();
            req.input("PAT_CD", patientCode);
            ...
            const insertedRow = Array.isArray(inserted?.recordset) && inserted.recordset.length > 0 ? inserted.recordset[0] : {};
            ...
            const headerWhere = hasTrNoCol && Number.isFinite(trNo) ? `PAT_CD = @PAT_CD AND ${trNoCol} = @TR_NO` : "PAT_CD = @PAT_CD";
        ...
        ORDER BY
          ...
          CASE WHEN ISDATE(ENTRYDATE) = 1 THEN CONVERT(datetime, ENTRYDATE) END DESC
      `);
            const hrow = Array.isArray(headerInfoRs?.recordset) && headerInfoRs.recordset.length > 0 ? headerInfoRs.recordset[0] : {};
        ...
        if (serviceCode && allowCreateFlowServiceInsert) {
            console.log(`[MSSQL Insert] Creating service row for ${patientCode} with code ${serviceCode}`);
            try {
                ...
                1
                    ...
                    "6",
                    "1",
                    ...(srvCols.has("SRV_BY1") && input.doctorCode ? [input.doctorCode] : []),
                ];
                const serviceExistsFilter = (() => {
                    ...
                    if (srvCols.has("UPDATEDATE")) {
                        recentChecks.push(`(ISDATE(UPDATEDATE) = 1 AND DATEDIFF(SECOND, CONVERT(datetime, UPDATEDATE), GETDATE()) BETWEEN 0 AND @DEDUP_SECONDS)`);
                    }
                    if (srvCols.has("ENTRYDATE")) {
                        recentChecks.push(`(ISDATE(ENTRYDATE) = 1 AND DATEDIFF(SECOND, CONVERT(datetime, ENTRYDATE), GETDATE()) BETWEEN 0 AND @DEDUP_SECONDS)`);
                    }
        ...
        `;
                ...
                console.error(`[MSSQL Service Insert Error] Patient ${patientCode}:`, err instanceof Error ? err.message : String(err));
            }
        }
        else {
            console.log(`[MSSQL Insert] Skipping service row: serviceCode=${serviceCode ? "set" : "empty"}, allowCreateFlowServiceInsert=${allowCreateFlowServiceInsert}`);
        }
        console.log(`[MSSQL Insert] ✅ Successfully inserted patient ${patientCode}!`);
        return { inserted: true, trNo: Number.isFinite(trNo) ? trNo : null };
    }
    catch (err) {
        console.error(`[MSSQL Insert Error] Patient ${patientCode}: ${err instanceof Error ? err.message : String(err)}`);
        console.error(`[MSSQL Insert Error] Stack:`, err instanceof Error ? err.stack : "");
        return { inserted: false, note: String(err instanceof Error ? err.message : err) };
    }
    finally {
        try {
            await pool.close();
...
async function upsertPatientToMssql(input) {
    const patientCode = String(input.patientCode ?? "").trim();
    const fullName = String(input.fullName ?? "").trim();
    if (!patientCode || !fullName) {
    ...
    `);
        ...
        await ensurePapatIoDefaults(pool, patientCode, Number.isFinite(Number(latestH?.TR_NO)) ? Math.trunc(Number(latestH?.TR_NO)) : null, Number.isFinite(Number(latestH?.VST_NO)) ? Math.trunc(Number(latestH?.VST_NO)) : 1, latestH?.DT ? new Date(latestH.DT) : new Date(todayDateOnly), doctorCode);
        await ensurePapatMfDefaults(pool, patientCode, new Date(todayDateOnly), fullName);
        if (serviceCode) {
            try {
                const srvCols = await getTableColumns(pool, "op2026.dbo.PAPAT_SRV");
...
async function ensurePatientServiceInMssql(patientCodeRaw, serviceCodeRaw, quantityRaw, doctorCodeRaw, doctorNameRaw) {
    const patientCode = String(patientCodeRaw ?? "").trim();
    ...
    `;
      ...
      `);
            const papatSrvCols = new Set((Array.isArray(papatSrvColsResult?.recordset) ? papatSrvColsResult.recordset : [])
                .map((r) => String(r?.name ?? "").toUpperCase())
                .filter(Boolean));
            const runSrvDoctorUpdate = async (column, value) => {
                const req = pool.request();
                req.input("PAT_CD", patientCode);
                req.input("SRV_CD", serviceCode);
                req.input("VAL", value);
        ...
        `);
                const targetCols = new Set((Array.isArray(targetColsResult?.recordset) ? targetColsResult.recordset : [])
                    .map((r) => String(r?.name ?? "").toUpperCase())
                    .filter(Boolean));
                const runHeaderDoctorUpdate = async (column) => {
                    const req = pool.request();
                    req.input("PAT_CD", patientCode);
          ...
          `);
                ...
                if (targetCols.has("DOC_NAM"))
                    await runHeaderDoctorUpdate("DOC_NAM");
                if (targetCols.has("DOC_NAME"))
                    await runHeaderDoctorUpdate("DOC_NAME");
            }
        }
...
async function deletePatientFromMssqlByCode(patientCodeRaw) {
    const patientCode = String(patientCodeRaw ?? "").trim();
    if (!patientCode)
        return { deleted: false, note: "Missing patientCode" };
    const enabled = asBool(process.env.MSSQL_PUSH_NEW_PATIENTS_ENABLED, true);
    if (!enabled) {
    ...
    try {
        ...
        const tryDeleteByCode = async (tableName) => {
            try {
...
async function backfillPapatSrvNamesInMssql(limitRaw) {
    const enabled = asBool(process.env.MSSQL_PUSH_NEW_PATIENTS_ENABLED, true);
    if (!enabled)
    ...
    try {
        ...
        if (canSetPatNmAr) {
            setParts.push(`PAT_NM_AR = CASE WHEN ISNULL(CONVERT(nvarchar(255), s.PAT_NM_AR), '') = '' THEN l.H_NAM ELSE s.PAT_NM_AR END`);
...
function getSyncQuery(limit, incrementalSince, includeServiceCode = true, includePatientSrvTable = true) {
...
async function loadDoctorMapFromCsv() {
    const now = Date.now();
    if (doctorCsvCache && now - doctorCsvCache.at < 60000)
        return doctorCsvCache.map;
    const configured = String(process.env.MSSQL_DOCTOR_MAP_CSV ?? "").trim();
    const csvPath = configured || node_path_1.default.join(process.cwd(), "MD.csv");
    const map = new Map();
    try {
...
async function loadServiceTypeMap() {
    const now = Date.now();
    if (serviceCsvCache && now - serviceCsvCache.at < 60000)
    ...
    try {
        ...
        if (setting?.value) {
            ...
            if (Array.isArray(parsed)) {
                for (const entry of parsed) {
                    const code = normalizeServiceCode(entry?.code);
                    const active = entry?.isActive !== false;
                    const normalized = normalizeServiceType(entry?.serviceType);
                    if (code && active && normalized)
    ...
    if (map.size === 0) {
        ...
        const csvPath = configured || node_path_1.default.join(process.cwd(), "srv.csv");
        try {
            const raw = await promises_1.default.readFile(csvPath, "utf8");
            const lines = raw.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
            for (let i = 1; i < lines.length; i += 1) {
...
async function loadDoctorServiceSheetMatchMap() {
    const now = Date.now();
    if (doctorServiceSheetMatchCache && now - doctorServiceSheetMatchCache.at < 60000) {
            ...
            if (Array.isArray(parsed)) {
                for (const entry of parsed) {
                    if (entry?.isActive === false)
                        continue;
                    const key = normalizeDoctorServiceMatchKey(entry?.doctorCode, entry?.serviceCode);
                    const sheet = normalizeSheetType(entry?.sheetType);
                    if (!key || !sheet)
                        continue;
                    map.set(key, sheet);
                }
...
async function getMssqlSyncStatus() {
    ...
    return {
        ...
        lastChangeCount: Number.isFinite(Number(runtime.lastChangeCount)) ? Number(runtime.lastChangeCount) : null,
    };
}
async function syncPatientsFromMssql(options = {}) {
    ...
    let runtime = {};
    try {
        runtime = runtimeRow?.value ? JSON.parse(String(runtimeRow.value)) : {};
    }
    catch {
        runtime = {};
    }
    const updateExisting = typeof runtime.overwriteExisting === "boolean"
        ? Boolean(runtime.overwriteExisting)
        : asBool(process.env.MSSQL_SYNC_UPDATE_EXISTING, false);
    const linkServicesForExisting = typeof runtime.linkServicesForExisting === "boolean"
        ? Boolean(runtime.linkServicesForExisting)
        : asBool(process.env.MSSQL_SYNC_LINK_SERVICES_FOR_EXISTING, true);
    const preserveManualEdits = typeof runtime.preserveManualEdits === "boolean"
    ...
    try {
        ...
        const rowsToProcess = rows.map((raw) => {
            ...
            if (serviceCode)
                mergedCodes.add(serviceCode);
            if (serviceCodesCsv) {
                for (const code of serviceCodesCsv.split(",").map((v) => normalizeServiceCode(v)).filter(Boolean)) {
        ...
        for (const row of rowsToProcess) {
            try {
                ...
                const nationalId = normalizeNationalId(pick(source, ["nationalId", "nid", "national_id"]));
                if (nationalId)
                ...
                if (primaryServiceCode)
                    payload.serviceCode = primaryServiceCode;
                ...
                const mappedMatch = (() => {
                    ...
                    for (const code of incomingServiceCodes) {
                        ...
                        if (byAny)
                            return { sheet: byAny, serviceCode: code };
                    }
                    return undefined;
                })();
                ...
                const serviceType = (mappedSheet ? normalizeServiceType(mappedSheet) : undefined) ||
                ...
                let locationType = normalizeLocationTypeFromIdNo(source.idno ?? source.IDNO) ??
                ...
                if (doctorCode) {
                    ...
                    try {
                        ...
                        const doctorRow = await dbConn
                            ?.select({ id: schema_1.doctorsLookup.id, name: schema_1.doctorsLookup.name })
                            .from(schema_1.doctorsLookup)
                            .where((0, drizzle_orm_1.eq)(schema_1.doctorsLookup.code, String(doctorCode)))
                            .limit(1);
                        if (doctorRow && doctorRow.length > 0) {
                            payload.doctorId = doctorRow[0].id;
                ...
                const fallbackDoctorName = String(doctorNameFromRow ?? "").trim() ||
                ...
                let existingExamStateData = null;
                if (existing) {
                    if (targetPatientId > 0 && preserveManualEdits) {
                        ...
                        existingExamStateData =
                            existingState && typeof existingState.data === "object" && existingState.data
                                ? existingState.data
                                : null;
                        manualLockEnabled = Boolean(existingExamStateData?.syncLockManual === true || String(existingExamStateData?.manualEditedAt ?? "").trim());
                    }
                    ...
                    if (!updateExisting) {
                        if (!dryRun && linkServicesForExisting && targetPatientId > 0 && incomingServiceCodes.length > 0) {
                            ...
                            const existingServiceCodesRaw = Array.isArray(existingData.serviceCodes)
                                ? existingData.serviceCodes
                                : [];
                            ...
                            const changed = Boolean(nextServiceCode) || mergedServiceCodes.length !== existingServiceCodes.length;
                            if (changed) {
                        ...
                        if (!manualLockEnabled) {
                            ...
                            const copyIfMissing = (key) => {
                                if (isBlank(payload[key]))
                                    return;
                                if (isBlank(existingRow[key]))
                                    nextUpdates[key] = payload[key];
                            };
                            const syncAlways = (key) => {
                                if (!isBlank(payload[key]))
                                    nextUpdates[key] = payload[key];
                            };
                            ...
                            if (!isBlank(payload.treatingDoctor)) {
                                ...
                                const currentDoctor = String(existing.treatingDoctor ?? "").trim();
                            ...
                            if (!isBlank(payload.lastVisit)) {
                                ...
                                const current = new Date(String(existing.lastVisit ?? ""));
                    ...
                    const mssqlEntryDate = source.ENTRYDATE || source.createdAt || source.UPDATEDATE;
                    const registrationDate = mssqlEntryDate ? new Date(String(mssqlEntryDate)) : new Date();
                    // تاريخ الكشف/الفحص - examination/visit date from MSSQL DT / VST_DT
                    const mssqlVisitDate = source.DT || source.VST_DT || source.VISIT_DT || source.TR_DT;
                ...
                if (!dryRun && targetPatientId > 0 && (doctorName || doctorCode || incomingServiceCodes.length > 0) && !manualLockEnabled) {
                    ...
                    const existingData = existingState && typeof existingState.data === "object" && existingState.data
                        ? existingState.data
                        : {};
                    const existingBackfill = existingData.mssqlBackfill && typeof existingData.mssqlBackfill === "object"
                        ? existingData.mssqlBackfill
                        : {};
                    const mergedBackfill = { ...existingBackfill };
                    let backfillChanged = false;
                    const alwaysRefreshBackfillKeys = new Set(["ENTRYDATE", "DT", "TR_DT", "secondDt"]);
                    for (const [k, v] of Object.entries(backfillFields)) {
                        if ((!isBlank(v) && alwaysRefreshBackfillKeys.has(k) && String(mergedBackfill[k] ?? "") !== String(v ?? "")) ||
                            (isBlank(mergedBackfill[k]) && !isBlank(v))) {
                            mergedBackfill[k] = v;
                            backfillChanged = true;
                        }
                    }
                    const existingDoctorName = String(existingData.doctorName ?? existingData.signatures?.doctor ?? "").trim();
                    const existingDoctorCode = String(existingData.doctorCode ?? "").trim();
                    const existingServiceCode = String(existingData.serviceCode ?? "").trim();
                    const existingDoctorCodesRaw = Array.isArray(existingData.doctorCodes)
                        ? existingData.doctorCodes
                        : [];
                    const existingDoctorNamesRaw = Array.isArray(existingData.doctorNames)
                        ? existingData.doctorNames
                        : [];
                    ...
                    const existingServiceCodes = existingServiceCodesRaw
                        .map((v) => String(v ?? "").trim())
                        .filter(Boolean);
                    const existingServiceSheetTypeByCode = existingData && typeof existingData.serviceSheetTypeByCode === "object"
                        ? existingData.serviceSheetTypeByCode
                        : {};
                    const mappedServiceSheetTypeByCode = {};
                    if (doctorCode) {
                        for (const code of incomingServiceCodes) {
                            const mapped = doctorServiceSheetMatchMap.get(normalizeDoctorServiceMatchKey(doctorCode, code));
                            if (mapped)
                                mappedServiceSheetTypeByCode[code] = mapped;
                        }
                    }
                    ...
                    const nextDoctorName = doctorName && doctorName !== existingDoctorName ? doctorName : "";
                    const nextDoctorCode = doctorCode && doctorCode !== existingDoctorCode ? doctorCode : "";
                    const mergedDoctorCodes = Array.from(new Set([...existingDoctorCodes, existingDoctorCode, doctorCode].filter(Boolean)));
                    const mergedDoctorNames = Array.from(new Set([...existingDoctorNames, existingDoctorName, doctorName].filter(Boolean)));
                    const nextServiceCode = !existingServiceCode ? primaryServiceCode : "";
                    const mergedServiceCodes = Array.from(new Set([...existingServiceCodes, existingServiceCode, ...incomingServiceCodes].filter(Boolean)));
                    if (!nextDoctorName &&
                        !nextDoctorCode &&
                    ...
                    await db.upsertPatientPageState(targetPatientId, "examination", {
                        ...
                        ...(mergedDoctorCodes.length > 0 ? { doctorCodes: mergedDoctorCodes } : {}),
                ...
                if (!dryRun && targetPatientId > 0 && incomingServiceCodes.length > 0) {
                    ...
                    const serviceDateIso = normalizeIsoDate(source.DT ?? source.VST_DT ?? source.VISIT_DT ?? source.TR_DT ?? source.ENTRYDATE ?? source.lastVisit) ||
                        undefined;
                    for (const code of incomingServiceCodes) {
        ...
        if (!dryRun && linkServicesForExisting) {
            try {
                const papatSrvCols = await getTableColumns(pool, "op2026.dbo.PAPAT_SRV");
        ...
        `;
                ...
                for (let index = 0; index < serviceRows.length; index += 1) {
                    ...
                    const serviceCode = normalizeServiceCode(pick(row, ["serviceCode", "SRV_CD", "srv_cd"]));
                    if (!patientCode || !serviceCode)
                        continue;
                    const trNo = Number.isFinite(Number(row?.trNo)) ? Math.trunc(Number(row.trNo)) : null;
                    const serviceDateIso = normalizeIsoDate(row?.serviceDate) || undefined;
                    const existing = await db.getPatientByCode(patientCode);
                    ...
                    const nextServiceCode = !existingServiceCode ? normalizeServiceCode(serviceCode) : "";
                    const changed = Boolean(nextServiceCode) || mergedServiceCodes.length !== existingServiceCodes.length;
                    if (changed) {
                        await db.upsertPatientPageState(targetPatientId, "examination", {
                            ...existingData,
        ...
        if (!dryRun) {
            ...
            if (result.inserted > 0) {
                const notificationSettings = await (0, appNotifications_1.getAppNotificationSettings)().catch(() => ({
                    mssqlOwnerEnabled: true,
                    mssqlInAppEnabled: true,
                    manualPatientInAppEnabled: true,
                }));
                const sampleLines = insertedPatientsSample.map(({ patientCode, fullName }) => `- ${patientCode} - ${String(fullName).trim()}`);
                const sampleSuffix = sampleLines.length > 0
                    ? `\n\nSample:\n${sampleLines.join("\n")}${result.inserted > insertedPatientsSample.length ? "\n- ..." : ""}`
                    : "";
                if (notificationSettings.mssqlOwnerEnabled) {
                    await (0, notification_1.notifyOwner)({
                        title: `MSSQL Sync: ${result.inserted} new patient${result.inserted === 1 ? "" : "s"} added`,
                        content: `Mode: ${incremental ? "incremental" : "full"}\n` +
                            `Fetched: ${result.fetched}\n` +
                            `Inserted: ${result.inserted}\n` +
                            `Updated: ${result.updated}\n` +
                            `Skipped: ${result.skipped}\n` +
                ...
                if (notificationSettings.mssqlInAppEnabled) {
                    await (0, appNotifications_1.pushAppNotification)({
                        title: `تمت إضافة ${result.inserted} مريض جديد من MSSQL`,
                        message: result.inserted === 1 && insertedPatientsSample[0]
