...
const OPERATIONAL_ONLY_KEYS = new Set([
  "changedAt",
  "serviceCodesCsv",
  "SRV_CODES",
  "srv_codes",
...
async function applyPapatSrvDefaults(
  ...
  if (cols.has("PA_VL")) {
    ...
    `);
    const discRow =
      Array.isArray(discRs?.recordset) && discRs.recordset.length > 0
        ? discRs.recordset[0]
        : {};
  ...
  try {
    ...
    if (Number.isFinite(vstNo)) {
      ...
      await pool
        ...
        .input("VST_NO", Math.trunc(vstNo))
        .input("TR_NO", Number.isFinite(trNo) ? Math.trunc(trNo) : null)
        .input("VISIT_DT", visitDt).query(`
        IF NOT EXISTS (
          SELECT 1
...
async function withMssqlServiceInsertLock(
  pool,
  patientCode,
  serviceCode,
  work,
...
async function ensurePapatIoDefaults(
  ...
  req.input(
    "TR_NO",
    Number.isFinite(Number(trNo)) ? Math.trunc(Number(trNo)) : null,
  );
  ...
  if (cols.has("DRS_CD") && doctorCode) {
    ioUpdates.push(
      `UPDATE op2026.dbo.PAPAT_IO SET DRS_CD = @DRS_CD WHERE PAT_CD = @PAT_CD AND VST_NO = @VST_NO;`,
    );
  }
  await req.query(`
    IF NOT EXISTS (
  ...
  if (cols.has("TOTL") || cols.has("DISC") || cols.has("PA_VL")) {
    ...
    if (cols.has("TOTL")) {
      await run(
        ...
        (req) => req.input("TOTL", Number.isFinite(totalSrv) ? totalSrv : 0),
      );
    }
    if (cols.has("DISC")) {
      await run(
    ...
    if (cols.has("PA_VL")) {
      await run(
        ...
        (req) => req.input("PAVL", Number.isFinite(totalSrv) ? totalSrv : 0),
      );
    }
  }
}
async function applyPajrnrReportDefaults(pool, targetTable, patientCode, trNo) {
  ...
  if (cols.has("CNCL")) {
    await run(
      `UPDATE ${targetTable} SET CNCL = ISNULL(CNCL, 0) WHERE ${whereClause}`,
    );
  }
}
...
async function ensurePatientServiceInMssql(
  patientCodeRaw,
  serviceCodeRaw,
  quantityRaw,
  doctorCodeRaw,
...
async function loadDoctorMapFromCsv() {
  ...
  const csvPath =
    configured || node_path_1.default.join(process.cwd(), "MD.csv");
  const map = new Map();
  try {
...
async function loadServiceTypeMap() {
  ...
  if (map.size === 0) {
    ...
    const csvPath =
      configured || node_path_1.default.join(process.cwd(), "srv.csv");
    try {
      const raw = await promises_1.default.readFile(csvPath, "utf8");
      const lines = raw
        .split(/\r?\n/)
...
async function loadDoctorServiceSheetMatchMap() {
  ...
  if (
    ...
    now - doctorServiceSheetMatchCache.at < 60000
  ) {
  ...
  try {
        ...
        if (doctorCode) {
          ...
          try {
            ...
            const doctorRow = await dbConn
              ?.select({
                id: schema_1.doctorsLookup.id,
                name: schema_1.doctorsLookup.name,
              })
              .from(schema_1.doctorsLookup)
              .where(
                (0, drizzle_orm_1.eq)(
                  schema_1.doctorsLookup.code,
                  String(doctorCode),
                ),
              )
              .limit(1);
        ...
        if (existing) {
          if (targetPatientId > 0 && preserveManualEdits) {
            ...
            existingExamStateData =
              existingState &&
              typeof existingState.data === "object" &&
              existingState.data
                ? existingState.data
                : null;
          ...
          if (!dryRun && targetPatientId > 0 && !manualLockEnabled) {
            const alwaysUpdates = {
              ...
              serviceType: isBlank(payload.serviceType)
                ? null
                : String(payload.serviceType),
              locationType: isBlank(payload.locationType)
                ? null
    ...
    if (!dryRun) {
      ...
      if (result.inserted > 0) {
        const notificationSettings = await (0,
        appNotifications_1.getAppNotificationSettings)().catch(() => ({
