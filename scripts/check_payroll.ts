...
async function check() {
  ...
  rows.forEach((r: any) => {
    ...
    console.log(
      `  ${r.emp_cd} (${isShift ? "SHIFT" : "EMP"}): basic=${r.basic_salary}, absent_ded=${r.absent_deduction}, total_ded=${r.total_deductions}, net=${r.net_basic}, pay=${r.total_pay}`,
    );
  });

  const shiftCount = rows.filter((r: any) =>
    r.emp_cd.startsWith("shift_"),
  ).length;
