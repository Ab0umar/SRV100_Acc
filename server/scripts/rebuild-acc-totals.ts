...
async function rebuildTable(
  ...
  for (const row of rows as any[]) {
    ...
    await db!.execute(
      sql.raw(`UPDATE ${table} SET total = ${rounded} WHERE id = ${row.id}`),
    );
    updated++;
  }
...
await rebuildTable(
  "accAdvances",
  (r) => r.prev + (Number(r.advance) || 0) - (Number(r.repayment) || 0),
);

// accHome: total = prev + inAmount - outAmount
await rebuildTable(
  "accHome",
  (r) => r.prev + (Number(r.inAmount) || 0) - (Number(r.outAmount) || 0),
);
...
await rebuildTable(
  "accLoans",
  (r) => r.prev + (Number(r.amount) || 0) - (Number(r.repayment) || 0),
);
