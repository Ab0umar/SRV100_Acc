      ...
      await db.insert(attendancePunches).values([
        {
          ...
          sourceHash: crypto
            .createHash("sha1")
            .update(`${empCd}|${inTime.getTime()}|in`)
            .digest("hex"),
        },
        {
          empCd,
          ...
          sourceHash: crypto
            .createHash("sha1")
            .update(`${empCd}|${outTime.getTime()}|out`)
            .digest("hex"),
        },
      ]);
    ...
    it("should handle bulk punch insertion", async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        ...
        punches.push({
          ...
          sourceHash: crypto
            .createHash("sha1")
            .update(`${empCd}|${punchAt.getTime()}|${direction}`)
            .digest("hex"),
        });
      }
  ...
  afterEach(async () => {
    ...
    if (db) {
      await db
        ...
        .where(eq(attendancePunches.empCd, "TEST001"))
        .catch(() => {});
      await db
        .delete(attendancePunches)
        .where(eq(attendancePunches.empCd, "TEST002"))
        .catch(() => {});
      await db
        .delete(attendancePunches)
        .where(eq(attendancePunches.empCd, "TEST003"))
        .catch(() => {});
    }
  });
});
