...
export const externalDoctorsRouter = router({
  ...
  createDoctor: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db.select({ id: externalDoctors.id }).from(externalDoctors)
        .where(eq(externalDoctors.username, input.username)).limit(1);
  ...
  updateDoctor: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (fields.doctorCode !== undefined) update.doctorCode = fields.doctorCode || null;
      if (fields.isActive !== undefined) update.isActive = fields.isActive;
      if (Object.keys(update).length === 0) return { success: true };
      await db.update(externalDoctors).set(update).where(eq(externalDoctors.id, id));
      return { success: true };
    }),
  ...
  resetPassword: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db.update(externalDoctors).set({ passwordHash } as any).where(eq(externalDoctors.id, input.id));
      return { success: true };
    }),
  ...
  listReferrals: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rows = await db
        ...
        .leftJoin(externalDoctors, eq(externalDoctorReferrals.externalDoctorId, externalDoctors.id))
        .where(
          input?.doctorId
            ? and(
                eq(externalDoctorReferrals.externalDoctorId, input.doctorId),
                input?.activeOnly ? eq(externalDoctorReferrals.isActive, true) : undefined,
              )
            : input?.activeOnly
      ...
      if (codes.length > 0) {
        const patientRows = await db
          .select({ patientCode: patients.patientCode, fullName: patients.fullName })
          .from(patients)
  ...
  createReferral: adminProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [doctor] = await db.select({ id: externalDoctors.id })
        .from(externalDoctors).where(eq(externalDoctors.id, input.externalDoctorId)).limit(1);
      if (!doctor) throw new TRPCError({ code: "NOT_FOUND", message: "External doctor not found" });

      // Verify patient exists
      const [patient] = await db.select({ id: patients.id })
        .from(patients).where(eq(patients.patientCode, input.patientCode)).limit(1);
      if (!patient) throw new TRPCError({ code: "NOT_FOUND", message: "Patient not found" });

      // Check for existing active referral
      const [existing] = await db.select({ id: externalDoctorReferrals.id })
        .from(externalDoctorReferrals)
        .where(
          and(
            eq(externalDoctorReferrals.externalDoctorId, input.externalDoctorId),
            eq(externalDoctorReferrals.patientCode, input.patientCode),
            eq(externalDoctorReferrals.isActive, true),
          ),
        )
        .limit(1);
      if (existing) throw new TRPCError({ code: "CONFLICT", message: "Referral already exists" });
  ...
  toggleReferral: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      await db.update(externalDoctorReferrals)
  ...
  deleteReferral: adminProcedure
    ...
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      await db.delete(externalDoctorReferrals).where(eq(externalDoctorReferrals.id, input.id));
      return { success: true };
    }),
  ...
  listAccessLogs: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        ...
        .leftJoin(externalDoctors, eq(externalDoctorAccessLogs.externalDoctorId, externalDoctors.id))
        .where(input?.doctorId ? eq(externalDoctorAccessLogs.externalDoctorId, input.doctorId) : undefined)
